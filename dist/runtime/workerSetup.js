...
const LEGACY_SIZE_BINS = Object.freeze([
...
const MAGNETIC_SIZE_SUITABILITY = Object.freeze({
    'lt-0.032mm': 0.05,
    '0.032-0.063mm': 0.10,
    '0.063-0.125mm': 0.15,
    '0.125-0.25mm': 0.20,
...
function compactRuntimeId(value, table, values, max, label) {
    const existing = table.get(value);
    if (existing != null)
        return existing;
    const next = values.length;
    if (next > max)
        throw new Error(`Flat runtime ${label} ID capacity exceeded.`);
    table.set(value, next);
    values.push(value);
    return next;
}
function textureRuntimeId(value, table, values) {
    if (value == null)
        return 0;
    const existing = table.get(value);
    if (existing != null)
    ...
    const next = values.length;
    if (next > 0xffff_ffff)
        throw new Error('Flat runtime texture ID capacity exceeded.');
    table.set(value, next);
    values.push(value);
    return next;
}
...
function runtimeSizeVocabulary() {
    return [
...
function derivedReactionTextureProfile(profile, reactionId, sourceSpeciesId, productSpeciesId) {
...
function reactionSpeciesMassKg(speciesId, stoichiometricMoles) {
    const chemistry = requireMaterialSpecies(speciesId).chemistry;
    if (!chemistry)
        throw new Error(`Reaction species '${speciesId}' requires chemistry data.`);
    return chemistry.molarMassKgPerMol * stoichiometricMoles;
}
function compileMaterialTables(plan, speciesTable, speciesIds, sizeBinTable, sizeBinIds, liberationTable, liberationClassIds, textureTable, textureProfileIds) {
    const sizes = runtimeSizeVocabulary();
    for (const size of sizes)
        compactRuntimeId(size.id, sizeBinTable, sizeBinIds, 0xff, 'particle-size');
    for (const item of LIBERATION_CLASSES)
        compactRuntimeId(item.id, liberationTable, liberationClassIds, 0xff, 'liberation');
    const sourceProfiles = new Map();
    const comminutionByProfile = new Map();
    for (const source of plan.resourceSources) {
        ...
        if (properties)
            comminutionByProfile.set(profile.id, properties);
    ...
    if (!kinetics || !thermochemistry)
        throw new Error(`Reaction '${reaction.id}' requires kinetics and thermochemistry.`);
    ...
    for (const profile of sourceProfiles.values()) {
        if (!profile.speciesTextures.goethite)
            continue;
        ...
        if (sourceProperties)
            comminutionByProfile.set(derived.id, { ...sourceProperties });
    }
    const species = speciesIds.map((canonicalId, runtimeId) => {
    ...
    const liberationClasses = LIBERATION_CLASSES.map((item, orderIndex) => ({
    ...
    const comminutionProperties = Array.from(comminutionByProfile.entries()).map(([canonicalTextureProfileId, properties]) => ({
    ...
    const sizeFactors = sizeBins.map(size => {
...
export function compileFlatWorkerSetup(plan) {
    ...
    const sizeBinIds = [];
    const liberationTable = new Map();
    const liberationClassIds = [];
    const textureTable = new Map([['untextured', 0]]);
    const textureProfileIds = ['untextured'];
    const occurrences = plan.resourceSources.map(source => {
        ...
        if (Math.abs(total - 1) > 1e-8)
            throw new Error(`Resource source '${source.sourceNodeId}' particulate populations must total 1 kg.`);
        return {
    ...
    const materialTables = compileMaterialTables(plan, speciesTable, speciesIds, sizeBinTable, sizeBinIds, liberationTable, liberationClassIds, textureTable, textureProfileIds);
    const machineByRuntimeId = new Map(plan.machines.map(machine => [machine.runtimeId, machine]));
    const hoppers = plan.machines.filter(machine => machine.nodeType === 'hopper').map(machine => ({
    ...
    const extractors = plan.machines.filter(machine => machine.nodeType === 'extractor').map((machine, ordinal) => {
