...
export function installNodeCatalogPanel(root, store) {
    const search = root.querySelector('#ws-node-catalog-search');
    const filters = root.querySelector('#ws-node-catalog-filters .ws-navigation-filters');
    const tree = root.querySelector('#ws-node-catalog-tree');
    const count = root.querySelector('#ws-node-catalog-match-count');
    const status = root.querySelector('#ws-node-catalog-status');
    if (!search || !filters || !tree || !count || !status)
        return;
