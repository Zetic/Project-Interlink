...
function createLandmassPolygon(seed, index, center, radiusX, radiusY) {
...
export function generateLandmasses(seed) {
    ...
    return LANDMASS_ANCHORS.slice(0, count).map((anchor, index) => {
        ...
        while (usedNames.has(name))
            name = rng.pick(LANDMASS_NAMES);
        usedNames.add(name);
        return { id: `landmass-${index}`, name, polygon, bounds: polygonBounds(polygon) };
    });
}
export function landmassContaining(landmasses, point) {
    for (const landmass of landmasses) {
        ...
        if (point.x < bounds.x || point.x > bounds.x + bounds.width || point.y < bounds.y || point.y > bounds.y + bounds.height)
            continue;
        if (pointInPolygon(point, landmass.polygon))
            return landmass;
    }
    return null;
}
function latticeValue(seed, namespace, x, y) {
    return createRng(seed, `${namespace}:${x}:${y}`).next();
}
function smooth(value) { return value * value * (3 - 2 * value); }
export function valueNoise(seed, namespace, point, scale) {
