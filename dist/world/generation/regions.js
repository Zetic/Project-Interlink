...
import { GEOGRAPHY_CELL_HEIGHT, GEOGRAPHY_CELL_WIDTH, GEOGRAPHY_COLUMNS, GEOGRAPHY_ROWS } from './geography.js';
...
function suffixForEnvironment(environment, ocean) {
    ...
    if (environment.tectonicActivity > 0.72)
        return 'Rift';
...
function createUniqueName(seed, cell, used) {
...
function coastal(cell, cells, dc, dr) {
    const column = cell.column + dc;
    const row = cell.row + dr;
    if (column < 0 || column >= GEOGRAPHY_COLUMNS || row < 0 || row >= GEOGRAPHY_ROWS)
        return true;
    return cells[row * GEOGRAPHY_COLUMNS + column].surfaceType !== cell.surfaceType;
}
function regionPolygon(cell, cells) {
    ...
    if (coastal(cell, cells, 0, -1))
        points.push({ x: (x0 + x1) / 2, y: y0 });
    points.push({ x: x1, y: y0 });
    if (coastal(cell, cells, 1, 0))
        points.push({ x: x1, y: (y0 + y1) / 2 });
    points.push({ x: x1, y: y1 });
    if (coastal(cell, cells, 0, 1))
        points.push({ x: (x0 + x1) / 2, y: y1 });
    points.push({ x: x0, y: y1 });
    if (coastal(cell, cells, -1, 0))
        points.push({ x: x0, y: (y0 + y1) / 2 });
    return points;
}
export function generateRegions(seed, cells) {
