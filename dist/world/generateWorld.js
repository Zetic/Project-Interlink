...
function createConnectorPath(seed, namespace, start, end, bendScale) {
    const rng = createRng(seed, namespace);
...
function createRegionName(seed, index, usedNames) {
...
function createRegions(seed) {
    ...
    const middleSplit = createConnectorPath(seed, 'geography:boundary:middle-split', westBoundary[MIDDLE_SPLIT_LEFT_INDEX], eastBoundary[MIDDLE_SPLIT_RIGHT_INDEX], 85);
    const rightEdgeSplit = {
        x: PLANET_MAP_WIDTH,
        y: roundCoordinate(PLANET_MAP_HEIGHT * createRng(seed, 'geography:right-edge-split').range(0.43, 0.55)),
    };
    const rightSplit = createConnectorPath(seed, 'geography:boundary:right-split', eastBoundary[RIGHT_SPLIT_BOUNDARY_INDEX], rightEdgeSplit, 95);
