...
import { environmentContextForPlanet, samplePlanetEnvironment } from '../world/generation/surfaceField.js';
...
const PLANET_OVERLAY_CACHE = new WeakMap();
const BASE_OPTIONS = [
...
function heat(value, low, high, alpha = 236) {
...
export function overlayLegendFor(id) {
    if (id === 'none')
        return null;
    if (id === 'elevation')
    ...
    if (id === 'sedimentary-tendency')
        return { title: 'Sedimentary Tendency', detail: 'Low → basin-prone', gradient: 'linear-gradient(90deg,#1d2a35,#6f6d5c,#c3a36b,#ead8a5)' };
    if (id === 'plate-boundaries')
        return { title: 'Plate Boundaries', detail: 'Convergent red · divergent cyan · transform violet' };
    if (id === 'tectonic-plates')
        return { title: 'Tectonic Plates', detail: 'Categorical plate identity' };
    if (id === 'crust-type')
...
export function overlayColorForEnvironment(id, sample, plateIndex = 0, crustType = 'continental') {
    if (id === 'elevation')
    ...
    if (id === 'sedimentary-tendency')
        return heat(sample.sedimentaryBasinFactor, [29, 42, 53], [235, 216, 165]);
    if (id === 'tectonic-plates')
        return [...hslToRgb(plateIndex * 137.508), 226];
    if (id === 'crust-type')
        return crustType === 'continental' ? [167, 143, 91, 232] : [48, 104, 151, 232];
    if (id === 'plate-boundaries') {
        if (sample.boundaryType === 'interior')
            return [0, 0, 0, 0];
        const alpha = byte(30 + sample.boundaryProximity * 225);
        if (sample.boundaryType === 'convergent')
            return [236, 78, 66, alpha];
        if (sample.boundaryType === 'divergent')
            return [72, 205, 223, alpha];
        return [180, 110, 216, alpha];
    }
    return [0, 0, 0, 0];
}
function overlayOption(id) {
    return MAP_OVERLAY_OPTIONS.find(option => option.id === id);
}
function cacheFor(planet) {
...
function environmentGridFor(planet) {
    const cache = cacheFor(planet);
    if (cache.environmentGrid)
    ...
    const plateById = new Map(planet.tectonicPlates.map((plate, index) => [plate.id, { plate, index }]));
...
function rasterFromPixels(width, height, pixel) {
...
function semanticRaster(planet) {
...
function continuousRaster(planet, id) {
    const grid = environmentGridFor(planet);
    if (id.startsWith('resource:')) {
        const resourceId = id.slice('resource:'.length);
        const context = environmentContextForPlanet(planet);
        const resourceColor = RESOURCE_COLORS[resourceId] ?? [245, 218, 116];
...
function buildOverlaySelect(select) {
    select.replaceChildren();
    const groups = ['None', 'Surface', 'Geology', 'Geography', 'Resources'];
...
function installOverlayControls(root) {
    const controls = root.querySelector('.ws-context-controls');
...
function updateLegend(element, id, hiddenForZoom) {
...
export function installMapOverlayRenderer(root, store) {
    const host = root.querySelector('#ws-map-canvas');
    const svg = root.querySelector('#ws-map-svg');
    const controls = installOverlayControls(root);
    if (!host || !svg || !controls)
        return;
    ...
    let currentCamera = store.getState().camera;
    let scheduled = false;
    let buildToken = 0;
    const draw = () => {
        scheduled = false;
    ...
    const renderSelectedOverlay = () => {
        ...
        currentOverlay = controls.select.value;
        ...
        window.setTimeout(() => {
            if (token !== buildToken || !currentPlanet)
                return;
