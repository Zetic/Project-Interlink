function defineSpecies(definition) {
    ...
    if (!definition.id)
        throw new Error('Material species id must be non-empty.');
    ...
    if (definition.chemistry) {
        ...
        for (const [element, atoms] of Object.entries(definition.chemistry.elementalComposition)) {
            if (!element || !Number.isFinite(atoms) || atoms <= 0)
                throw new Error(`Material species '${definition.id}' has invalid elemental composition.`);
        }
    }
...
export function materialSpeciesById(speciesId) {
...
export function requireMaterialSpecies(speciesId) {
    ...
    if (!species)
        throw new Error(`Unsupported material species '${speciesId}'.`);
    return species;
}
