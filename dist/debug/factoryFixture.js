...
import { connectPorts, placeMechanicalNode, removeMechanicalNode, setMechanicalNodeEnabled, setMechanicalNodeParameter, } from '../graph/graphCommands.js';
import { metersToWorldUnits } from '../world/scale.js';
export const DEBUG_FACTORY_FEEDER_RATE_KG_PER_SECOND = 0.2;
function requiredDefinition(id) {
    const definition = apparatusDefinitionById(id);
    if (!definition)
        throw new Error(`Debug factory requires apparatus definition '${id}'.`);
    return definition;
}
function portById(node, portId) {
    const port = node.ports.find(candidate => candidate.id === portId);
    if (!port)
        throw new Error(`Debug factory node '${node.id}' has no port '${portId}'.`);
    return port;
}
function connect(graph, source, sourcePortId, target, targetPortId) {
    return connectPorts(graph, { nodeId: source.id, portId: sourcePortId }, portById(source, sourcePortId), { nodeId: target.id, portId: targetPortId }, portById(target, targetPortId));
}
function offset(origin, xMeters, yMeters) {
    return {
        x: origin.x + metersToWorldUnits(xMeters),
        y: origin.y + metersToWorldUnits(yMeters),
    };
}
function addNode(graph, definitionId, position, { enabled = false, parameterId = null, parameterValue = null } = {}) {
    const placed = placeMechanicalNode(graph, requiredDefinition(definitionId), position);
    let nextGraph = placed.graph;
    if (enabled)
        nextGraph = setMechanicalNodeEnabled(nextGraph, placed.node.id, true);
    ...
    if (!node)
        throw new Error(`Debug factory failed to place '${definitionId}'.`);
    return { graph: nextGraph, node };
}
function placeOneFactory(graph, resource, origin) {
    let nextGraph = graph;
    const nodes = [];
    const place = (definitionId, xMeters, yMeters, options = {}) => {
...
export function placeDebugProcessingFactories(graph, resource, count = 1) {
...
export function removeDebugProcessingFactories(graph, manifests) {
    let nextGraph = graph;
    for (const manifest of manifests) {
        for (const nodeId of manifest.nodeIds)
            nextGraph = removeMechanicalNode(nextGraph, nodeId);
    }
    return nextGraph;
}
