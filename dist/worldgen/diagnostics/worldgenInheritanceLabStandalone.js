...
function rotate(position, yaw, pitch) {
...
function samplePosition(result, sample) {
...
function renderPlanet(canvas, result, projection, mode, yaw, pitch) {
    const width = 1100;
    ...
    for (let sample = 0; sample < result.metrics.fineSampleCount; sample += 1) {
        ...
        if (mode === 'plates' || boundaryMode)
            ...
            context.fillStyle = provenanceColor(result.nearestCoarseSource[sample]);
        else if (mode === 'inherited-mask')
            context.fillStyle = result.inheritedSampleMask[sample] ? '#f4e27a' : '#5794c8';
        else if (scalarDescriptor)
            context.fillStyle = scalarColor(scalarDescriptor.values[sample], scalarDescriptor.minimum, scalarDescriptor.maximum, scalarDescriptor.lowHue, scalarDescriptor.highHue);
        else
    ...
    if (boundaryMode) {
        ...
        for (let boundary = 0; boundary < result.metrics.fineBoundaryEdgeCount; boundary += 1) {
            ...
            if (mode === 'tectonic-boundaries')
                context.strokeStyle = tectonicBoundaryColor(result.boundaryKinds[boundary]);
            else if (mode === 'geological-boundaries')
                context.strokeStyle = geologicalBoundaryColor(result.geologicalBoundaryRegimes[boundary]);
            else
                context.strokeStyle = provenanceColor(result.boundaryCoarseSourceIndices[boundary]);
...
canvas.addEventListener('pointerdown', event => { if (projection.value !== 'globe')
    return; drag = { x: event.clientX, y: event.clientY, yaw, pitch }; canvas.setPointerCapture(event.pointerId); });
canvas.addEventListener('pointermove', event => { if (!drag || projection.value !== 'globe')
    return; yaw = drag.yaw + (event.clientX - drag.x) * 0.007; pitch = Math.max(-1.45, Math.min(1.45, drag.pitch + (event.clientY - drag.y) * 0.007)); redraw(); });
canvas.addEventListener('pointerup', event => { drag = null; if (canvas.hasPointerCapture(event.pointerId))
