...
impl PackedSpeciesThermalTable {
    ...
    pub fn heat_capacity_j_per_k(&self, state: &PackedSolidState) -> Result<f64, String> {
        ...
        for index in 0..state.len() {
            let quantity = state.quantity_at(index).expect("packed columns share one length");
            if quantity <= SOLID_MATERIAL_TOLERANCE {
                continue;
            }
            let descriptor = state.descriptor_at(index).expect("packed columns share one length");
            capacity += quantity * self.specific_heat_capacity_j_per_kg_k(descriptor.species_id)?;
        }
...
fn output_specific_sensible_enthalpies(
    ...
    let delta_temperature_k = total_input_sensible_enthalpy_j / total_input_heat_capacity_j_per_k;
...
impl PackedSplitterConfig {
    pub fn new(split_fraction_to_a: f64, throughput_kg_per_second: f64, enabled: bool) -> Result<Self, String> {
        ...
        Ok(Self { split_fraction_to_a, throughput_kg_per_second, enabled })
    }
}
...
impl PackedSplitterRuntime {
    ...
    pub fn set_enabled(&mut self, enabled: bool) {
        ...
        if !enabled { self.operating_state = PackedOperatingState::Off; }
        else if self.operating_state == PackedOperatingState::Off { self.operating_state = PackedOperatingState::Idle; }
    }

    pub fn input_stream(&self) -> &PackedSolidStream { &self.input_stream }
    pub fn output_a_stream(&self) -> &PackedSolidStream { &self.output_a_stream }
    pub fn output_b_stream(&self) -> &PackedSolidStream { &self.output_b_stream }
    pub fn operating_state(&self) -> PackedOperatingState { self.operating_state }
    pub fn last_error(&self) -> Option<&str> { self.last_error.as_deref() }

    fn finish(&mut self, state: PackedOperatingState, transferred_mass_kg: f64, output_a_mass_kg: f64, output_b_mass_kg: f64) -> PackedSplitterTickResult {
        self.operating_state = state;
        PackedSplitterTickResult { operating_state: state, transferred_mass_kg, output_a_mass_kg, output_b_mass_kg }
    }
    ...
    pub fn tick_hopper_to_hoppers(
        ...
        if !self.config.enabled { return Ok(self.finish(PackedOperatingState::Off, 0.0, 0.0, 0.0)); }
        let stored_mass_kg = source.stored_mass_kg();
        if stored_mass_kg <= SOLID_MATERIAL_TOLERANCE { return Ok(self.finish(PackedOperatingState::Idle, 0.0, 0.0, 0.0)); }

        let candidate_rate = self.config.throughput_kg_per_second.min(stored_mass_kg / dt);
        ...
        let scale_a = if requested_a_kg <= APPARATUS_TRANSFER_TOLERANCE_KG { 1.0 } else { output_a.free_capacity_kg() / requested_a_kg };
        let scale_b = if requested_b_kg <= APPARATUS_TRANSFER_TOLERANCE_KG { 1.0 } else { output_b.free_capacity_kg() / requested_b_kg };
        ...
        let output_specific = output_specific_sensible_enthalpies(&[&withdrawal.body], &[&output_a_flow, &output_b_flow], thermal)?;
        ...
        *output_a = staged_a;
        *output_b = staged_b;
        self.input_stream.set_flow(&actual_feed, withdrawal.body.specific_sensible_enthalpy_j_per_kg())?;
        self.output_a_stream.set_flow(&output_a_flow, output_specific[0])?;
        self.output_b_stream.set_flow(&output_b_flow, output_specific[1])?;
        Ok(self.finish(PackedOperatingState::Running, withdrawal.actual_mass_kg, accepted_a, accepted_b))
    }
}
...
impl PackedMergerConfig {
    pub fn new(throughput_kg_per_second: f64, enabled: bool) -> Result<Self, String> {
        ...
        Ok(Self { throughput_kg_per_second, enabled })
    }
}
...
impl PackedMergerRuntime {
    ...
    pub fn input_a_stream(&self) -> &PackedSolidStream { &self.input_a_stream }
    pub fn input_b_stream(&self) -> &PackedSolidStream { &self.input_b_stream }
    pub fn output_stream(&self) -> &PackedSolidStream { &self.output_stream }
    pub fn operating_state(&self) -> PackedOperatingState { self.operating_state }
    pub fn last_error(&self) -> Option<&str> { self.last_error.as_deref() }

    fn finish(&mut self, state: PackedOperatingState, input_a_mass_kg: f64, input_b_mass_kg: f64, output_mass_kg: f64) -> PackedMergerTickResult {
        self.operating_state = state;
        PackedMergerTickResult { operating_state: state, input_a_mass_kg, input_b_mass_kg, output_mass_kg }
    }
    ...
    pub fn tick_hoppers_to_hopper(
        ...
        if total_stored <= SOLID_MATERIAL_TOLERANCE { return Ok(self.finish(PackedOperatingState::Idle, 0.0, 0.0, 0.0)); }

        let candidate_total_rate = self.config.throughput_kg_per_second.min(total_stored / dt);
        ...
        let capacity_scale = if requested_output_kg <= APPARATUS_TRANSFER_TOLERANCE_KG { 1.0 } else { (output.free_capacity_kg() / requested_output_kg).min(1.0).max(0.0) };
        if capacity_scale <= APPARATUS_TRANSFER_TOLERANCE_KG {
        ...
        let withdrawal_a = staged_a.withdraw_rate(candidate_rate_a * capacity_scale, dt)?;
        let withdrawal_b = staged_b.withdraw_rate(candidate_rate_b * capacity_scale, dt)?;
        ...
        let product_specific = output_specific_sensible_enthalpies(&[&withdrawal_a.body, &withdrawal_b.body], &[&product], thermal)?[0];
        ...
        *output = staged_output;
        self.input_a_stream.set_flow(&actual_a, withdrawal_a.body.specific_sensible_enthalpy_j_per_kg())?;
        self.input_b_stream.set_flow(&actual_b, withdrawal_b.body.specific_sensible_enthalpy_j_per_kg())?;
        self.output_stream.set_flow(&product, product_specific)?;
        Ok(self.finish(PackedOperatingState::Running, withdrawal_a.actual_mass_kg, withdrawal_b.actual_mass_kg, accepted_output))
    }
}
...
mod tests {
    ...
    fn hopper_with(species_id: u16, mass_kg: f64, sensible_enthalpy_j: f64, capacity_kg: f64) -> PackedHopperState {
        let mut state = PackedSolidState::new();
        state.push_fraction(FractionDescriptor { species_id, size_bin_id: 1, liberation_class_id: 1, texture_profile_id: 0 }, mass_kg).unwrap();
        PackedHopperState::new(capacity_kg, PackedSolidBody::new(state, sensible_enthalpy_j).unwrap()).unwrap()
    }

    fn thermal_table() -> PackedSpeciesThermalTable {
        let mut table = PackedSpeciesThermalTable::new();
        table.set_specific_heat_capacity_j_per_kg_k(1, 500.0).unwrap();
        table.set_specific_heat_capacity_j_per_kg_k(2, 1000.0).unwrap();
        table
    }
    ...
    fn splitter_routes_mass_energy_and_streams_conservatively() {
        ...
        let mut splitter = PackedSplitterRuntime::new(PackedSplitterConfig::new(0.25, 8.0, true).unwrap());
        let result = splitter.tick_hopper_to_hoppers(&mut source, &mut output_a, &mut output_b, &thermal_table(), 0.1).unwrap();
    ...
    fn splitter_throttles_all_outputs_by_the_tightest_capacity() {
        ...
        let mut output_b = PackedHopperState::empty(100.0).unwrap();
        let mut splitter = PackedSplitterRuntime::new(PackedSplitterConfig::new(0.5, 10.0, true).unwrap());
        let result = splitter.tick_hopper_to_hoppers(&mut source, &mut output_a, &mut output_b, &thermal_table(), 0.1).unwrap();
    ...
    #[test]
    fn merger_mixes_species_and_thermal_states_at_equilibrium() {
        ...
        let mut merger = PackedMergerRuntime::new(PackedMergerConfig::new(10.0, true).unwrap());
        let before_energy = input_a.body().sensible_enthalpy_j() + input_b.body().sensible_enthalpy_j();
        let result = merger.tick_hoppers_to_hopper(&mut input_a, &mut input_b, &mut output, &thermal_table(), 0.1).unwrap();
        assert_eq!(result.operating_state, PackedOperatingState::Running);
        ...
        let after_energy = input_a.body().sensible_enthalpy_j() + input_b.body().sensible_enthalpy_j() + output.body().sensible_enthalpy_j();
