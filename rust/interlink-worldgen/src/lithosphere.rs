use crate::{random, CrustKind, CrustalModel, PlanetTopology, StageIdentity, TectonicModel, WorldgenError};
...
impl LithosphereMetrics {
    pub fn lithosphere_hash_hex(&self) -> String { format!("{:016x}", self.lithosphere_hash) }
}
...
fn hash_u8(mut hash: u64, values: &[u8]) -> u64 { hash = fnv_update(hash, &(values.len() as u64).to_le_bytes()); fnv_update(hash, values) }
fn hash_u16(mut hash: u64, values: &[u16]) -> u64 { hash = fnv_update(hash, &(values.len() as u64).to_le_bytes()); for value in values { hash = fnv_update(hash, &value.to_le_bytes()); } hash }
fn hash_f32(mut hash: u64, values: &[f32]) -> u64 { hash = fnv_update(hash, &(values.len() as u64).to_le_bytes()); for value in values { hash = fnv_update(hash, &value.to_bits().to_le_bytes()); } hash }
...
fn clamp_signed(value: f64) -> f64 { value.clamp(-1.0, 1.0) }
fn norm(value: [f64; 3]) -> f64 { (value[0] * value[0] + value[1] * value[1] + value[2] * value[2]).sqrt() }
fn normalize(value: [f64; 3]) -> [f64; 3] { let magnitude = norm(value).max(1.0e-15); [value[0] / magnitude, value[1] / magnitude, value[2] / magnitude] }
...
fn smooth_random_field<T: PlanetTopology>(topology: &T, seed: u64, passes: usize) -> Vec<f64> {
    ...
    let maximum = values.iter().map(|value| value.abs()).fold(0.0_f64, f64::max).max(1.0e-12);
    for value in &mut values { *value /= maximum; }
    values
}
...
fn build_mechanical_state<T: PlanetTopology>(
    ...
    mechanical_seed: u64,
    mantle_seed: u64,
) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<u8>, Vec<f32>) {
    ...
    for sample in 0..count {
        ...
        let deformation_penalty = strain * 0.34 + rift * 0.25 + transform * 0.20 + subduction * 0.12;
        ...
        let strength_index = clamp01(
            base_strength + age_factor * 0.24 + thickness_factor * 0.10 + inherited * 0.055
                - thermal_index.max(0.0) * 0.24 - deformation_penalty,
        );
        let weakness_index = clamp01(
            1.0 - strength_index + strain * 0.16 + rift * 0.12 + transform * 0.08,
        );
        let te_km = (5.0 + strength_index * 73.0 + age_factor * 8.0 - thermal_index.max(0.0) * 9.0).clamp(4.0, 86.0);
        let upwelling_index = clamp01(0.16 + mantle.max(0.0) * 0.58 + ridge * 0.52 + rift * 0.22 - subduction * 0.12);
        let support_index = clamp_signed(mantle * 0.52 + upwelling_index * 0.34 + ridge * 0.22 - subduction * 0.18);
        let column_buoyancy = clamp_signed(f64::from(geology.buoyancy_index[sample]) * 0.82 + support_index * 0.18);

        let margin = if crust == CrustKind::Transitional as u8 { 0.58 } else { 0.0 };
        let suture = if crust != CrustKind::Oceanic as u8 { orogeny * (0.55 + strain * 0.45) } else { 0.0 };
        ...
        let propensity = clamp01(
            weakness_index * 0.42 + strain * 0.26 + maximum_fabric * 0.20
    ...
    (strength, weakness, elastic_thickness, thermal, upwelling, dynamic_support, compensated_buoyancy, fabric_strength, structural_kind, fragmentation_propensity)
}
...
fn collect_fragment_components<T: PlanetTopology>(
    ...
    let count = topology.sample_count() as usize;
    let eligible = (0..count).map(|sample| {
    ...
    for start in 0..count as u32 {
        ...
        if visited[start_index] || !eligible[start_index] { continue; }
        ...
        while let Some(sample) = queue.pop_front() {
            ...
            for neighbor in topology.neighbors(sample) {
                ...
                if !visited[index] && eligible[index] && tectonics.plate_ids[index] == parent_plate_id {
                    visited[index] = true;
        ...
        let area = samples.iter().map(|sample| topology.area_steradians(*sample)).sum::<f64>();
        let mean_weakness = samples.iter().map(|sample| f64::from(weakness[*sample as usize])).sum::<f64>() / samples.len() as f64;
        let mean_propensity = samples.iter().map(|sample| f64::from(propensity[*sample as usize])).sum::<f64>() / samples.len() as f64;
        let seed_sample = *samples.iter().max_by(|left, right| {
            propensity[**left as usize].total_cmp(&propensity[**right as usize]).then_with(|| right.cmp(left))
        }).expect("fragment component is non-empty");
        components.push(FragmentComponent { samples, parent_plate_id, area_steradians: area, mean_weakness, mean_propensity, seed_sample });
    }
    components.sort_by(|left, right| right.mean_propensity.total_cmp(&left.mean_propensity)
...
fn build_refinement<T: PlanetTopology>(
    ...
    let components = collect_fragment_components(topology, tectonics, strength, weakness, fabric, propensity, geology);

    for component in components {
        if fragments.len() >= MAX_TECTONIC_FRAGMENTS { break; }
        ...
        if parent_fraction > 0.24 || accepted_area + component.area_steradians > maximum_fragmented_area { continue; }
        let kind = if component.samples.len() >= 4
            && parent_fraction >= 0.006
            && parent_fraction <= 0.16
            && component.mean_propensity >= 0.62
        { TectonicFragmentKind::Microplate } else { TectonicFragmentKind::Terrane };
        ...
        let angular_velocity = if kind == TectonicFragmentKind::Microplate {
            ...
            let perturb_direction = random_unit_vector(refinement_seed, u64::from(component.seed_sample) ^ u64::from(fragment_id));
...
fn model_hash(model: &LithosphericModel) -> u64 {
    ...
    for fragment in &model.fragments {
        ...
        for value in fragment.angular_velocity_rad_per_myr { hash = fnv_update(hash, &value.to_bits().to_le_bytes()); }
    }
    hash
}

fn validate_model<T: PlanetTopology>(topology: &T, tectonics: &TectonicModel, model: &LithosphericModel) -> Result<(), WorldgenError> {
    let count = topology.sample_count() as usize;
    let lengths = [
        model.strength_index.len(), model.weakness_index.len(), model.effective_elastic_thickness_km.len(),
        model.thermal_anomaly_index.len(), model.mantle_upwelling_index.len(), model.mantle_dynamic_support_index.len(),
        model.compensated_buoyancy_index.len(), model.structural_fabric_strength.len(), model.structural_zone_kind.len(),
        model.fragmentation_propensity.len(), model.fragment_ids.len(), model.kinematic_domain_ids.len(),
    ];
    if lengths.iter().any(|length| *length != count) { return Err(WorldgenError::InvalidLithosphere("lithosphere fields do not match topology sample count")); }
    for sample in 0..count {
        let unit_fields = [model.strength_index[sample], model.weakness_index[sample], model.mantle_upwelling_index[sample], model.structural_fabric_strength[sample], model.fragmentation_propensity[sample]];
        if unit_fields.iter().any(|value| !value.is_finite() || *value < 0.0 || *value > 1.0) { return Err(WorldgenError::InvalidLithosphere("lithosphere normalized field is outside [0,1]")); }
        let signed_fields = [model.thermal_anomaly_index[sample], model.mantle_dynamic_support_index[sample], model.compensated_buoyancy_index[sample]];
        if signed_fields.iter().any(|value| !value.is_finite() || *value < -1.0 || *value > 1.0) { return Err(WorldgenError::InvalidLithosphere("lithosphere signed field is outside [-1,1]")); }
        if !model.effective_elastic_thickness_km[sample].is_finite() || model.effective_elastic_thickness_km[sample] < 4.0 || model.effective_elastic_thickness_km[sample] > 86.0 { return Err(WorldgenError::InvalidLithosphere("effective elastic thickness is outside supported bounds")); }
        let fragment_id = model.fragment_ids[sample];
        if fragment_id > 0 {
            let fragment = model.fragments.get(fragment_id as usize - 1).ok_or(WorldgenError::InvalidLithosphere("fragment field references missing fragment"))?;
            if tectonics.plate_ids[sample] != fragment.parent_plate_id { return Err(WorldgenError::InvalidLithosphere("tectonic fragment crosses a macro-plate boundary")); }
        }
    }
    if model.fragments.len() > MAX_TECTONIC_FRAGMENTS { return Err(WorldgenError::InvalidLithosphere("too many tectonic fragments")); }
    if model.metrics.fragmented_area_fraction > 0.220_001 { return Err(WorldgenError::InvalidLithosphere("tectonic refinement is not selective")); }
    Ok(())
}
...
pub fn generate_lithosphere<T: PlanetTopology>(
    ...
    if tectonics.plate_ids.len() != topology.sample_count() as usize || geology.crust_kind.len() != topology.sample_count() as usize {
        return Err(WorldgenError::InvalidLithosphere("upstream tectonic/geology state does not match lithosphere topology"));
    }
    ...
    let (strength_index, weakness_index, effective_elastic_thickness_km, thermal_anomaly_index, mantle_upwelling_index, mantle_dynamic_support_index, compensated_buoyancy_index, structural_fabric_strength, structural_zone_kind, fragmentation_propensity) =
    ...
    let (fragment_ids, kinematic_domain_ids, fragments) = build_refinement(
        topology, tectonics, geology, &strength_index, &weakness_index, &structural_fabric_strength, &fragmentation_propensity, refinement_seed,
    );
    let total_area = (0..topology.sample_count()).map(|sample| topology.area_steradians(sample)).sum::<f64>();
    let fragmented_area = (0..topology.sample_count()).filter(|sample| fragment_ids[*sample as usize] > 0).map(|sample| topology.area_steradians(sample)).sum::<f64>();
    let microplates = fragments.iter().filter(|fragment| fragment.kind == TectonicFragmentKind::Microplate).count() as u16;
    ...
    let mut metrics = LithosphereMetrics {
        ...
        mean_effective_elastic_thickness_km: weighted_mean(topology, &effective_elastic_thickness_km),
        ...
        suture_sample_count: structural_zone_kind.iter().filter(|kind| **kind == StructuralZoneKind::Suture as u8).count() as u32,
        rift_zone_sample_count: structural_zone_kind.iter().filter(|kind| **kind == StructuralZoneKind::Rift as u8).count() as u32,
        transform_zone_sample_count: structural_zone_kind.iter().filter(|kind| **kind == StructuralZoneKind::Transform as u8).count() as u32,
        continental_margin_sample_count: structural_zone_kind.iter().filter(|kind| **kind == StructuralZoneKind::ContinentalMargin as u8).count() as u32,
    ...
    let mut model = LithosphericModel {
        stage: StageIdentity { id: LITHOSPHERE_STAGE_ID, version: LITHOSPHERE_STAGE_VERSION, derived_seed: stage_seed },
        mechanical_seed, mantle_seed, refinement_seed,
        strength_index, weakness_index, effective_elastic_thickness_km, thermal_anomaly_index, mantle_upwelling_index,
        mantle_dynamic_support_index, compensated_buoyancy_index, structural_fabric_strength, structural_zone_kind,
        fragmentation_propensity, fragment_ids, kinematic_domain_ids, fragments, metrics: metrics.clone(),
    };
...
mod tests {
    use super::*;
    use crate::{build_icosphere, generate_crust_and_history, generate_tectonics, GeologyRequest, PlanetPhysicalParameters, TectonicsRequest};

    fn generate(seed: &str, level: u8, plate_count: u16) -> (crate::GeodesicTopology, TectonicModel, CrustalModel, LithosphericModel) {
        let topology = build_icosphere(level).unwrap();
        let parameters = PlanetPhysicalParameters::earthlike_reference();
        let tectonics = generate_tectonics(&topology, &TectonicsRequest::new(seed, plate_count), parameters).unwrap();
        let geology = generate_crust_and_history(&topology, &tectonics, &GeologyRequest::new(seed), parameters).unwrap();
        let lithosphere = generate_lithosphere(&topology, &tectonics, &geology, &LithosphereRequest::new(seed)).unwrap();
    ...
    fn lithosphere_is_deterministic_and_seed_sensitive() {
        ...
        assert_eq!(first.metrics.lithosphere_hash, second.metrics.lithosphere_hash);
        assert_eq!(first.strength_index, second.strength_index);
        assert_ne!(first.metrics.lithosphere_hash, changed.metrics.lithosphere_hash);
    }
    ...
    fn stable_old_crust_is_stronger_than_high_strain_crust_in_ensemble() {
        ...
        for seed in ["wg3-5-a", "wg3-5-b", "wg3-5-c", "wg3-5-d", "wg3-5-e"] {
            ...
            for sample in 0..geology.crust_kind.len() {
                if geology.crust_kind[sample] == CrustKind::Continental as u8 && geology.crust_age_myr[sample] > 1500.0 && geology.crustal_strain[sample] < 0.18 {
        ...
        assert!(microplate_total > 0, "ensemble should exercise selective microplate refinement");
    }
}
