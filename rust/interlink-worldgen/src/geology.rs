use crate::{random, PlateBoundaryKind, PlanetPhysicalParameters, PlanetTopology, StageIdentity, TectonicModel, WorldgenError};
...
impl GeologyRequest {
    pub fn new(seed: impl Into<String>) -> Self { Self { seed: seed.into() } }
}
...
impl GeologyMetrics {
    pub fn geology_hash_hex(&self) -> String { format!("{:016x}", self.geology_hash) }
}
...
impl CrustalModel {
    pub fn boundary_regimes(&self) -> Vec<u8> { self.boundaries.iter().map(|edge| edge.regime as u8).collect() }
    pub fn subduction_polarities(&self) -> Vec<u8> { self.boundaries.iter().map(|edge| edge.subduction_polarity as u8).collect() }
    pub fn plate_scale_classes(&self) -> Vec<u8> { self.plate_summaries.iter().map(|plate| plate.scale_class as u8).collect() }
}
...
impl PartialEq for DistanceFrontier {
    fn eq(&self, other: &Self) -> bool {
        self.distance_rad.to_bits() == other.distance_rad.to_bits() && self.source == other.source && self.sample == other.sample
    }
}
...
impl PartialOrd for DistanceFrontier { fn partial_cmp(&self, other: &Self) -> Option<Ordering> { Some(self.cmp(other)) } }

fn dot(a: [f64; 3], b: [f64; 3]) -> f64 { a[0] * b[0] + a[1] * b[1] + a[2] * b[2] }
fn arc_radians(a: [f64; 3], b: [f64; 3]) -> f64 { dot(a, b).clamp(-1.0, 1.0).acos() }
fn unit_random(value: u64) -> f64 { ((random::mix64(value) >> 11) as f64) * (1.0 / 9_007_199_254_740_992.0) }
...
fn smooth_random_field<T: PlanetTopology>(topology: &T, seed: u64, passes: usize) -> Vec<f64> {
    let mut values = (0..topology.sample_count())
        .map(|sample| unit_random(seed ^ u64::from(sample).wrapping_mul(0x9e37_79b9_7f4a_7c15)) * 2.0 - 1.0)
        .collect::<Vec<_>>();
    let mut next = vec![0.0; values.len()];
    for _ in 0..passes {
        for sample in 0..topology.sample_count() {
            let neighbors = topology.neighbors(sample);
            let mean = neighbors.iter().map(|neighbor| values[*neighbor as usize]).sum::<f64>() / neighbors.len() as f64;
            next[sample as usize] = values[sample as usize] * 0.42 + mean * 0.58;
        }
        std::mem::swap(&mut values, &mut next);
    }
    let max_abs = values.iter().map(|value| value.abs()).fold(0.0_f64, f64::max).max(1.0e-12);
    for value in &mut values { *value /= max_abs; }
    values
}
...
fn select_craton_seeds<T: PlanetTopology>(topology: &T, count: usize, seed: u64) -> Vec<u32> {
    ...
    for index in 0..count {
        ...
        for attempt in 0..sample_count.clamp(512, 4096) {
            ...
            let distance = if selected.is_empty() { PI } else {
                selected.iter().map(|other| arc_radians(position, topology.unit_position(*other))).fold(PI, f64::min)
            };
            if distance > fallback_distance { fallback = Some(candidate); fallback_distance = distance; }
            if distance >= minimum_separation { accepted = Some(candidate); break; }
        }
        if accepted.is_none() {
        ...
        selected.push(accepted.or(fallback).expect("craton seed selection requires a topology sample"));
    }
    selected
}

fn weighted_descending_threshold<T: PlanetTopology>(topology: &T, values: &[f64], target_fraction: f64) -> f64 {
    let mut order = (0..values.len()).collect::<Vec<_>>();
    order.sort_by(|left, right| values[*right].total_cmp(&values[*left]).then_with(|| left.cmp(right)));
    let total = (0..topology.sample_count()).map(|sample| topology.area_steradians(sample)).sum::<f64>();
    let target = total * target_fraction;
    let mut accumulated = 0.0;
    for index in order {
        accumulated += topology.area_steradians(index as u32);
        if accumulated >= target { return values[index]; }
    }
...
fn build_crust_partition<T: PlanetTopology>(topology: &T, tectonics: &TectonicModel, province_seed: u64) -> (Vec<u8>, Vec<u16>, Vec<u32>, Vec<f64>, Vec<f64>) {
    ...
    for index in 0..cratons.len() {
        let radius = 0.36 + unit_random(province_seed ^ (index as u64).wrapping_mul(0xa076_1d64_78bd_642f)) * 0.42;
        let age = 900.0 + unit_random(province_seed ^ (index as u64).wrapping_mul(0xe703_7ed1_a0b4_28db) ^ 0x8ebc_6af0_9c88_c6e3) * 2400.0;
        radii.push(radius);
    ...
    let continental_threshold = weighted_descending_threshold(topology, &affinity, continental_target);
    let transition_threshold = weighted_descending_threshold(topology, &affinity, (continental_target + transitional_target).min(0.50));
...
fn initial_properties<T: PlanetTopology>(topology: &T, kinds: &[u8], provinces: &[u16], craton_ages: &[f64], property_seed: u64) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>) {
    ...
    for sample in 0..topology.sample_count() {
        ...
        let (age, thickness, density) = match kind {
            CrustKind::Continental => {
                ...
                (age, (32.0 + oldness * 10.0 + noise * 2.0).clamp(28.0, 46.0), (2810.0 - oldness * 95.0 + noise * 18.0).clamp(2670.0, 2840.0))
            }
            CrustKind::Transitional => {
                let base = craton_ages[provinces[index] as usize];
                let age = (220.0 + base * 0.24 + noise * 120.0).clamp(120.0, 1050.0);
                (age, (22.0 + noise * 3.5).clamp(16.0, 29.0), (2880.0 + noise * 25.0).clamp(2820.0, 2930.0))
            }
            CrustKind::Oceanic => {
...
fn classify_regime(kind_a: CrustKind, kind_b: CrustKind, buoyancy_a: f64, buoyancy_b: f64, tectonic_kind: PlateBoundaryKind) -> (GeologicalBoundaryRegime, SubductionPolarity) {
    match tectonic_kind {
        PlateBoundaryKind::Transform => (GeologicalBoundaryRegime::Transform, SubductionPolarity::None),
        PlateBoundaryKind::Divergent => {
            if matches!(kind_a, CrustKind::Oceanic) && matches!(kind_b, CrustKind::Oceanic) {
                (GeologicalBoundaryRegime::OceanicRidge, SubductionPolarity::None)
            } else if matches!(kind_a, CrustKind::Continental) && matches!(kind_b, CrustKind::Continental) {
                (GeologicalBoundaryRegime::ContinentalRift, SubductionPolarity::None)
            } else {
                (GeologicalBoundaryRegime::TransitionalDivergence, SubductionPolarity::None)
            }
        }
        PlateBoundaryKind::Convergent => {
            match (kind_a, kind_b) {
                (CrustKind::Continental, CrustKind::Continental) => (GeologicalBoundaryRegime::ContinentalCollision, SubductionPolarity::None),
                (CrustKind::Oceanic, CrustKind::Continental) | (CrustKind::Oceanic, CrustKind::Transitional) => (GeologicalBoundaryRegime::OceanContinentSubduction, SubductionPolarity::PlateA),
                (CrustKind::Continental, CrustKind::Oceanic) | (CrustKind::Transitional, CrustKind::Oceanic) => (GeologicalBoundaryRegime::OceanContinentSubduction, SubductionPolarity::PlateB),
                (CrustKind::Oceanic, CrustKind::Oceanic) => {
                    let polarity = if buoyancy_a <= buoyancy_b { SubductionPolarity::PlateA } else { SubductionPolarity::PlateB };
                ...
                _ => {
                    if (buoyancy_a - buoyancy_b).abs() > 0.28 {
                        ...
                        (GeologicalBoundaryRegime::ContinentalCollision, SubductionPolarity::None)
                    }
                }
            }
        }
...
fn build_geological_boundaries(kinds: &[u8], buoyancies: &[f32], tectonics: &TectonicModel) -> Vec<GeologicalBoundary> {
    tectonics.boundaries.iter().map(|edge| {
        let kind_a = crust_kind(kinds[edge.sample_a as usize]);
        let kind_b = crust_kind(kinds[edge.sample_b as usize]);
        let (regime, polarity) = classify_regime(kind_a, kind_b, f64::from(buoyancies[edge.sample_a as usize]), f64::from(buoyancies[edge.sample_b as usize]), edge.kind);
        GeologicalBoundary {
...
fn multi_source_distance<T: PlanetTopology>(topology: &T, sources: &[u32], allowed: Option<&[bool]>) -> (Vec<f64>, Vec<u32>) {
    ...
    for (source, sample) in sources.iter().enumerate() {
        if let Some(mask) = allowed { if !mask[*sample as usize] { continue; } }
        if distance[*sample as usize] == 0.0 { continue; }
        distance[*sample as usize] = 0.0;
        source_id[*sample as usize] = source as u32;
        frontier.push(DistanceFrontier { distance_rad: 0.0, source: source as u32, sample: *sample });
    }
    while let Some(current) = frontier.pop() {
        let index = current.sample as usize;
        if current.distance_rad > distance[index] + 1.0e-14 || current.source != source_id[index] { continue; }
        ...
        for neighbor_index in 0..neighbors.len() {
            ...
            if let Some(mask) = allowed { if !mask[neighbor as usize] { continue; } }
            ...
            if candidate + 1.0e-14 < distance[target] {
                ...
                frontier.push(DistanceFrontier { distance_rad: candidate, source: current.source, sample: neighbor });
            }
        }
    }
    (distance, source_id)
...
fn update_oceanic_ages<T: PlanetTopology>(topology: &T, parameters: PlanetPhysicalParameters, kinds: &[u8], boundaries: &[GeologicalBoundary], history_seed: u64, ages: &mut [f32], thicknesses: &mut [f32], densities: &mut [f32], buoyancies: &mut [f32]) {
    let allowed = kinds.iter().map(|kind| !matches!(crust_kind(*kind), CrustKind::Continental)).collect::<Vec<_>>();
    let mut sources = Vec::new();
    let mut source_rates = Vec::new();
    for boundary in boundaries {
        if !matches!(boundary.regime, GeologicalBoundaryRegime::OceanicRidge | GeologicalBoundaryRegime::TransitionalDivergence) { continue; }
    ...
    for sample in 0..topology.sample_count() {
        let index = sample as usize;
        if !matches!(crust_kind(kinds[index]), CrustKind::Oceanic) { continue; }
...
fn influence_field<T: PlanetTopology>(topology: &T, sources: &[u32], scale_rad: f64) -> Vec<f32> {
    if sources.is_empty() { return vec![0.0; topology.sample_count() as usize]; }
    let (distance, _) = multi_source_distance(topology, sources, None);
    distance.into_iter().map(|value| if value.is_finite() { (-value / scale_rad).exp().clamp(0.0, 1.0) as f32 } else { 0.0 }).collect()
}

fn history_fields<T: PlanetTopology>(topology: &T, parameters: PlanetPhysicalParameters, kinds: &[u8], ages: &[f32], boundaries: &[GeologicalBoundary], history_seed: u64) -> (Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>, Vec<f32>) {
    ...
    for boundary in boundaries {
        match boundary.regime {
            GeologicalBoundaryRegime::ContinentalCollision => { orogen_sources.extend_from_slice(&[boundary.sample_a, boundary.sample_b]); }
            GeologicalBoundaryRegime::OceanicSubduction | GeologicalBoundaryRegime::OceanContinentSubduction => {
                subduction_sources.extend_from_slice(&[boundary.sample_a, boundary.sample_b]);
                match boundary.subduction_polarity {
                    SubductionPolarity::PlateA => { trench_sources.push(boundary.sample_a); arc_sources.push(boundary.sample_b); orogen_sources.push(boundary.sample_b); }
                    SubductionPolarity::PlateB => { trench_sources.push(boundary.sample_b); arc_sources.push(boundary.sample_a); orogen_sources.push(boundary.sample_a); }
                    SubductionPolarity::None => {}
                }
            }
            GeologicalBoundaryRegime::ContinentalRift | GeologicalBoundaryRegime::TransitionalDivergence => { rift_sources.extend_from_slice(&[boundary.sample_a, boundary.sample_b]); }
            GeologicalBoundaryRegime::OceanicRidge => { ridge_sources.extend_from_slice(&[boundary.sample_a, boundary.sample_b]); }
            GeologicalBoundaryRegime::Transform => { transform_sources.extend_from_slice(&[boundary.sample_a, boundary.sample_b]); }
        }
    }
    ...
    for sample in 0..topology.sample_count() as usize {
        let continental_weight = match crust_kind(kinds[sample]) { CrustKind::Continental => 1.0, CrustKind::Transitional => 0.55, CrustKind::Oceanic => 0.0 };
        let inherited = clamp01((inherited_orogen[sample] + 0.45) / 1.45) * continental_weight * 0.34;
        orogen[sample] = f64::max(f64::from(orogen[sample]), inherited) as f32;
        let inherited_extension = clamp01((-inherited_orogen[sample] + 0.25) / 1.25) * continental_weight * 0.13;
    ...
    for sample in 0..kinds.len() {
        let ocean_age = if matches!(crust_kind(kinds[sample]), CrustKind::Oceanic) { f64::from(ages[sample]) / 220.0 } else { 0.0 };
        let subsidence_value = clamp01(f64::from(rift[sample]) * 0.56 + f64::from(trench[sample]) * 0.38 + ocean_age * 0.34);
        subsidence[sample] = subsidence_value as f32;
        let stable = 1.0 - f64::from(orogen[sample]).max(f64::from(rift[sample])).max(f64::from(subduction[sample]));
        let inherited = clamp01((inherited_basin[sample] + 0.55) / 1.55);
        let crust_weight = match crust_kind(kinds[sample]) { CrustKind::Continental => 1.0, CrustKind::Transitional => 0.85, CrustKind::Oceanic => 0.20 };
        basin[sample] = clamp01((subsidence_value * 0.52 + stable.max(0.0) * 0.22 + inherited * 0.32) * crust_weight) as f32;
...
fn apply_history_to_crust(kinds: &[u8], orogen: &[f32], rift: &[f32], thicknesses: &mut [f32], densities: &mut [f32], buoyancies: &mut [f32]) {
    for index in 0..kinds.len() {
        let original = f64::from(thicknesses[index]);
        let adjusted = match crust_kind(kinds[index]) {
            CrustKind::Continental => (original + f64::from(orogen[index]) * 8.5 - f64::from(rift[index]) * 5.5).clamp(25.0, 56.0),
            CrustKind::Transitional => (original + f64::from(orogen[index]) * 4.5 - f64::from(rift[index]) * 6.0).clamp(12.0, 34.0),
...
fn boundary_interface_length<T: PlanetTopology>(topology: &T, sample_a: u32, sample_b: u32) -> f64 {
    ...
    neighbors.iter().position(|neighbor| *neighbor == sample_b).map(|index| interfaces[index]).unwrap_or(0.0)
}

fn build_plate_summaries<T: PlanetTopology>(topology: &T, tectonics: &TectonicModel, kinds: &[u8], ages: &[f32], thicknesses: &[f32]) -> Vec<PlateSummary> {
    ...
    tectonics.plates.iter().map(|plate| {
        ...
        let scale_class = if area >= mean_area * 1.5 { PlateScaleClass::Major } else if area <= mean_area * 0.65 { PlateScaleClass::Minor } else { PlateScaleClass::Intermediate };
        PlateSummary {
            plate_id: plate.id,
            ...
            convergent_boundary_fraction: if boundary_total > 0.0 { convergent[index] / boundary_total } else { 0.0 },
            divergent_boundary_fraction: if boundary_total > 0.0 { divergent[index] / boundary_total } else { 0.0 },
            transform_boundary_fraction: if boundary_total > 0.0 { transform[index] / boundary_total } else { 0.0 },
        }
    }).collect()
}
...
fn geology_hash(stage_seed: u64, model: &CrustalModel) -> u64 {
    ...
    for value in &model.crust_province_id { hash = fnv1a_update(hash, &value.to_le_bytes()); }
    for field in [&model.crust_age_myr, &model.crust_thickness_km, &model.crust_density_kg_per_m3, &model.buoyancy_index, &model.orogenic_history, &model.rift_history, &model.ridge_history, &model.subduction_history, &model.trench_history, &model.volcanic_arc_history, &model.transform_history, &model.subsidence_history, &model.basin_potential, &model.crustal_strain] {
        for value in field { hash = fnv1a_update(hash, &value.to_bits().to_le_bytes()); }
    }
    for boundary in &model.boundaries {
        hash = fnv1a_update(hash, &boundary.sample_a.to_le_bytes());
        hash = fnv1a_update(hash, &boundary.sample_b.to_le_bytes());
        hash = fnv1a_update(hash, &[boundary.regime as u8, boundary.subduction_polarity as u8]);
    }
    for plate in &model.plate_summaries { hash = fnv1a_update(hash, &[plate.scale_class as u8]); }
    hash
}

fn compute_metrics<T: PlanetTopology>(topology: &T, kinds: &[u8], ages: &[f32], thicknesses: &[f32], boundaries: &[GeologicalBoundary], hash: u64) -> GeologyMetrics {
    let total_area = (0..topology.sample_count()).map(|sample| topology.area_steradians(sample)).sum::<f64>();
    let mut areas = [0.0_f64; 3];
    let mut age_sum = [0.0_f64; 3];
    let mut thickness_sum = [0.0_f64; 3];
    for sample in 0..topology.sample_count() {
        let index = sample as usize;
        let bucket = match crust_kind(kinds[index]) { CrustKind::Oceanic => 0, CrustKind::Transitional => 1, CrustKind::Continental => 2 };
        let area = topology.area_steradians(sample);
        areas[bucket] += area;
    ...
    for boundary in boundaries { regime_counts[boundary.regime as usize - 1] += 1; }
    GeologyMetrics {
        ...
        mean_continental_age_myr: if areas[2] > 0.0 { age_sum[2] / areas[2] } else { 0.0 },
        mean_oceanic_age_myr: if areas[0] > 0.0 { age_sum[0] / areas[0] } else { 0.0 },
        mean_continental_thickness_km: if areas[2] > 0.0 { thickness_sum[2] / areas[2] } else { 0.0 },
        mean_oceanic_thickness_km: if areas[0] > 0.0 { thickness_sum[0] / areas[0] } else { 0.0 },
...
fn validate_model<T: PlanetTopology>(topology: &T, tectonics: &TectonicModel, model: &CrustalModel) -> Result<(), WorldgenError> {
    let count = topology.sample_count() as usize;
    let fields: [&[f32]; 14] = [&model.crust_age_myr, &model.crust_thickness_km, &model.crust_density_kg_per_m3, &model.buoyancy_index, &model.orogenic_history, &model.rift_history, &model.ridge_history, &model.subduction_history, &model.trench_history, &model.volcanic_arc_history, &model.transform_history, &model.subsidence_history, &model.basin_potential, &model.crustal_strain];
    if model.crust_kind.len() != count || model.crust_province_id.len() != count || fields.iter().any(|field| field.len() != count) { return Err(WorldgenError::InvalidGeology("crust fields do not match topology sample count")); }
    if model.boundaries.len() != tectonics.boundaries.len() { return Err(WorldgenError::InvalidGeology("geological boundary interpretation lost tectonic edges")); }
    if model.plate_summaries.len() != tectonics.plates.len() { return Err(WorldgenError::InvalidGeology("plate summaries do not match tectonic plate count")); }
    if fields.iter().flat_map(|field| field.iter()).any(|value| !value.is_finite()) { return Err(WorldgenError::InvalidGeology("geological fields contain non-finite values")); }
    if model.crust_age_myr.iter().any(|value| *value < 0.0 || *value > 4000.0) { return Err(WorldgenError::InvalidGeology("crust ages leave the supported physical range")); }
    if model.crust_thickness_km.iter().any(|value| *value < 4.0 || *value > 60.0) { return Err(WorldgenError::InvalidGeology("crust thickness leaves the supported physical range")); }
    if model.crust_density_kg_per_m3.iter().any(|value| *value < 2500.0 || *value > 3200.0) { return Err(WorldgenError::InvalidGeology("crust density leaves the supported physical range")); }
    let fraction_sum = model.metrics.continental_area_fraction + model.metrics.transitional_area_fraction + model.metrics.oceanic_area_fraction;
    if (fraction_sum - 1.0).abs() > 1.0e-10 { return Err(WorldgenError::InvalidGeology("crust area fractions do not close to the sphere")); }
    Ok(())
}

pub fn generate_crust_and_history<T: PlanetTopology>(topology: &T, tectonics: &TectonicModel, request: &GeologyRequest, parameters: PlanetPhysicalParameters) -> Result<CrustalModel, WorldgenError> {
    parameters.validate().map_err(WorldgenError::InvalidParameters)?;
    if tectonics.plate_ids.len() != topology.sample_count() as usize { return Err(WorldgenError::InvalidGeology("tectonic ownership does not match geology topology")); }
    ...
    let (kinds, provinces, _cratons, craton_ages, _affinity) = build_crust_partition(topology, tectonics, province_seed);
    let (mut ages, mut thicknesses, mut densities, mut buoyancies) = initial_properties(topology, &kinds, &provinces, &craton_ages, property_seed);
    let initial_boundaries = build_geological_boundaries(&kinds, &buoyancies, tectonics);
    update_oceanic_ages(topology, parameters, &kinds, &initial_boundaries, history_seed, &mut ages, &mut thicknesses, &mut densities, &mut buoyancies);
    let mut boundaries = build_geological_boundaries(&kinds, &buoyancies, tectonics);
    let (orogen, rift, ridge, subduction, trench, arc, transform, subsidence, basin, strain) = history_fields(topology, parameters, &kinds, &ages, &boundaries, history_seed);
    apply_history_to_crust(&kinds, &orogen, &rift, &mut thicknesses, &mut densities, &mut buoyancies);
    ...
    let stage = StageIdentity { id: GEOLOGY_STAGE_ID, version: GEOLOGY_STAGE_VERSION, derived_seed: stage_seed };
    let placeholder_metrics = GeologyMetrics { sample_count: topology.sample_count(), continental_area_fraction: 0.0, transitional_area_fraction: 0.0, oceanic_area_fraction: 0.0, mean_continental_age_myr: 0.0, mean_oceanic_age_myr: 0.0, mean_continental_thickness_km: 0.0, mean_oceanic_thickness_km: 0.0, oceanic_subduction_edges: 0, ocean_continent_subduction_edges: 0, continental_collision_edges: 0, oceanic_ridge_edges: 0, continental_rift_edges: 0, transitional_divergence_edges: 0, transform_edges: 0, geology_hash: 0 };
    let mut model = CrustalModel { stage, province_seed, property_seed, history_seed, crust_kind: kinds, crust_province_id: provinces, crust_age_myr: ages, crust_thickness_km: thicknesses, crust_density_kg_per_m3: densities, buoyancy_index: buoyancies, orogenic_history: orogen, rift_history: rift, ridge_history: ridge, subduction_history: subduction, trench_history: trench, volcanic_arc_history: arc, transform_history: transform, subsidence_history: subsidence, basin_potential: basin, crustal_strain: strain, boundaries, plate_summaries, metrics: placeholder_metrics };
    let hash = geology_hash(stage_seed, &model);
    model.metrics = compute_metrics(topology, &model.crust_kind, &model.crust_age_myr, &model.crust_thickness_km, &model.boundaries, hash);
...
mod tests {
    ...
    fn world(seed: &str) -> (crate::GeodesicTopology, TectonicModel, CrustalModel) {
        ...
        let tectonics = generate_tectonics(&topology, &TectonicsRequest::new(seed, 14), parameters).unwrap();
        let geology = generate_crust_and_history(&topology, &tectonics, &GeologyRequest::new(seed), parameters).unwrap();
    ...
    fn crust_generation_is_deterministic_complete_and_mixed_within_plates() {
        ...
        let repeat = generate_crust_and_history(&topology, &tectonics, &GeologyRequest::new("wg3-determinism"), PlanetPhysicalParameters::earthlike_reference()).unwrap();
        ...
        assert!(geology.plate_summaries.iter().any(|plate| plate.continental_fraction > 0.05 && plate.oceanic_fraction > 0.05));
    }
    ...
    fn geological_boundaries_preserve_every_tectonic_edge_and_assign_causal_regimes() {
        ...
        for (tectonic, geological) in tectonics.boundaries.iter().zip(&geology.boundaries) {
            assert_eq!((tectonic.sample_a, tectonic.sample_b), (geological.sample_a, geological.sample_b));
            match tectonic.kind {
                PlateBoundaryKind::Transform => assert_eq!(geological.regime, GeologicalBoundaryRegime::Transform),
                PlateBoundaryKind::Divergent => assert!(matches!(geological.regime, GeologicalBoundaryRegime::OceanicRidge | GeologicalBoundaryRegime::ContinentalRift | GeologicalBoundaryRegime::TransitionalDivergence)),
                PlateBoundaryKind::Convergent => assert!(matches!(geological.regime, GeologicalBoundaryRegime::OceanicSubduction | GeologicalBoundaryRegime::OceanContinentSubduction | GeologicalBoundaryRegime::ContinentalCollision)),
            }
        }
    }
    ...
    fn derived_plate_scale_classes_are_area_based_and_do_not_replace_physical_area() {
        ...
        for summary in &geology.plate_summaries {
            let expected = if summary.area_steradians >= mean * 1.5 { PlateScaleClass::Major } else if summary.area_steradians <= mean * 0.65 { PlateScaleClass::Minor } else { PlateScaleClass::Intermediate };
            assert_eq!(summary.scale_class, expected);
            assert!((summary.continental_fraction + summary.transitional_fraction + summary.oceanic_fraction - 1.0).abs() < 1.0e-8);
        }
    }
    ...
    fn history_fields_are_bounded_and_crust_responds_to_orogeny_and_rifting() {
        ...
        for field in [&geology.orogenic_history, &geology.rift_history, &geology.ridge_history, &geology.subduction_history, &geology.trench_history, &geology.volcanic_arc_history, &geology.transform_history, &geology.subsidence_history, &geology.basin_potential, &geology.crustal_strain] {
            assert!(field.iter().all(|value| value.is_finite() && *value >= 0.0 && *value <= 1.0));
        }
        let mut orogenic_continental = Vec::new();
        let mut quiet_continental = Vec::new();
        for index in 0..geology.crust_kind.len() {
            if crust_kind(geology.crust_kind[index]) != CrustKind::Continental { continue; }
            if geology.orogenic_history[index] > 0.65 { orogenic_continental.push(geology.crust_thickness_km[index]); }
            if geology.orogenic_history[index] < 0.18 && geology.rift_history[index] < 0.18 { quiet_continental.push(geology.crust_thickness_km[index]); }
        }
        if !orogenic_continental.is_empty() && !quiet_continental.is_empty() {
            let orogenic_mean = orogenic_continental.iter().map(|value| f64::from(*value)).sum::<f64>() / orogenic_continental.len() as f64;
            let quiet_mean = quiet_continental.iter().map(|value| f64::from(*value)).sum::<f64>() / quiet_continental.len() as f64;
