...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct SyntheticRequest { pub seed: String, pub width: u32, pub height: u32 }
impl SyntheticRequest { pub fn new(seed: impl Into<String>, width: u32, height: u32) -> Self { Self { seed: seed.into(), width, height } } }
#[derive(Clone, Debug, PartialEq)]
pub struct SyntheticDiagnostic { pub generator_version: u32, pub stage: StageIdentity, pub parameters: PlanetPhysicalParameters, pub field: DenseU16Field, pub statistics: FieldStatistics }
...
pub fn generate_synthetic(request: &SyntheticRequest) -> Result<SyntheticDiagnostic, WorldgenError> {
    ...
    for y in 0..request.height { let lat_shape = triangular_wave(y, request.height); for x in 0..request.width { let lon_shape = triangular_wave(x, request.width); let noise = (random::coordinate_value(stage_seed, x, y) & 0xffff) as u32; let structured = (u32::from(u16::MAX) - lat_shape) / 2 + lon_shape / 3; values.push(((noise * 3 + structured * 2) / 5).min(u32::from(u16::MAX)) as u16); } }
    let field = DenseU16Field::from_values(request.width, request.height, values)?; let statistics = field.statistics();
    Ok(SyntheticDiagnostic { generator_version: WORLDGEN_ENGINE_VERSION, stage: StageIdentity { id: SYNTHETIC_STAGE_ID, version: SYNTHETIC_STAGE_VERSION, derived_seed: stage_seed }, parameters, field, statistics })
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn synthetic_generation_is_deterministic_and_seed_sensitive() { let request = SyntheticRequest::new("wg0-regression", 96, 48); let a = generate_synthetic(&request).unwrap(); let b = generate_synthetic(&request).unwrap(); assert_eq!(a.statistics.hash, b.statistics.hash); assert_eq!(a.field.values(), b.field.values()); let changed = generate_synthetic(&SyntheticRequest::new("wg0-regression-b", 96, 48)).unwrap(); assert_ne!(a.statistics.hash, changed.statistics.hash); }
    #[test] fn synthetic_generation_reports_dense_field_statistics() { let result = generate_synthetic(&SyntheticRequest::new("stats", 32, 16)).unwrap(); assert_eq!(result.generator_version, WORLDGEN_ENGINE_VERSION); assert_eq!(result.stage.id, SYNTHETIC_STAGE_ID); assert_eq!(result.statistics.sample_count, 512); assert!(result.statistics.minimum <= result.statistics.maximum); assert!(result.statistics.mean.is_finite()); assert_eq!(result.statistics.hash_hex().len(), 16); }
}
