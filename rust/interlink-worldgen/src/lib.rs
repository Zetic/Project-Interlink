...
pub const WORLDGEN_ENGINE_VERSION: u32 = 1;
...
impl fmt::Display for WorldgenError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidDimensions(message) | Self::InvalidParameters(message) => formatter.write_str(message),
        }
    }
}
...
impl SyntheticRequest {
    pub fn new(seed: impl Into<String>, width: u32, height: u32) -> Self {
        Self { seed: seed.into(), width, height }
    }
}
...
fn triangular_wave(index: u32, period: u32) -> u32 {
    ...
    if scaled <= u32::from(u16::MAX) { scaled } else { 2 * u32::from(u16::MAX) - scaled }
}

/// WG-0 proof field. This is intentionally not terrain. It exists only to prove
/// deterministic dense-field generation, stage isolation, Worker/WASM transport,
/// diagnostics, and native execution before physical algorithms are introduced.
pub fn generate_synthetic(request: &SyntheticRequest) -> Result<SyntheticDiagnostic, WorldgenError> {
    ...
    Ok(SyntheticDiagnostic {
        ...
        stage: StageIdentity { id: SYNTHETIC_STAGE_ID, version: SYNTHETIC_STAGE_VERSION, derived_seed: stage_seed },
        parameters,
        field,
        statistics,
    })
...
mod tests {
    ...
    fn synthetic_generation_is_deterministic_and_seed_sensitive() {
        ...
        let changed = generate_synthetic(&SyntheticRequest::new("wg0-regression-b", 96, 48)).unwrap();
