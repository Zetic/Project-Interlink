...
function candidateCountForRegion(region: Region): number {
...
export function generateResourceFeatures(seed: string, regions: Region[], context: PlanetEnvironmentContext): ResourceNode[] {
  ...
  if (!iron || landRegions.length === 0) throw new Error('Generator v7 requires land and an Iron Ore definition.');
