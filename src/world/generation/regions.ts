...
function edgeKey(start: Point, end: Point): string {
  const a = pointKey(start); const b = pointKey(end);
  return a < b ? `${a}|${b}` : `${b}|${a}`;
}
...
function frozenParentBoundaryVertices(patches: readonly GeographyPatch[]): Set<string> {
  ...
  for (const patch of patches) {
    for (let index = 0; index < patch.polygon.length; index += 1) {
      ...
      else owners.set(key, { count: 1, start, end });
    }
  }
  const frozen = new Set<string>();
  for (const edge of owners.values()) if (edge.count === 1) {
...
function warpedRegionPoint(seed: string, point: Point, frozen: ReadonlySet<string>, cellWidth: number, cellHeight: number): Point {
  ...
  if (frozen.has(key)) return roundPoint(point);
