...
import type { GeographicRegionType, GeographicTrait, Point, SurfaceType } from '../types.js';
...
export interface GeographicProvinceSeed {
  id: string;
  patchIndex: number;
  point: Point;
  parentId: string;
...
export interface GeographicProvinceAssignment {
  seed: GeographicProvinceSeed;
...
const SIGNIFICANT_TYPES = new Set<GeographicRegionType>([
...
const GENERIC_TYPES = new Set<GeographicRegionType>([
...
function uniqueTraits(values: readonly GeographicTrait[]): GeographicTrait[] {
  return [...new Set(values)].sort();
}

function buildPatchGraph(patches: readonly GeographyPatch[]): PatchGraph {
  const neighbors = Array.from({ length: patches.length }, () => [] as number[]);
  ...
  for (let cursor = 0; cursor < queue.length; cursor += 1) {
    ...
    if (nextDistance > 12) continue;
    for (const neighbor of neighbors[current]!) {
      if (nextDistance >= coastDistance[neighbor]!) continue;
...
function traitsForEnvironment(environment: PlanetEnvironmentSample, surfaceType: SurfaceType, coastDistance: number): GeographicTrait[] {
  const traits: GeographicTrait[] = [];
  if (coastDistance <= 2) traits.push('coastal');
  if (environment.boundaryType === 'convergent' && environment.boundaryProximity > 0.42) traits.push('orogenic');
  if (environment.volcanicActivity > 0.58) traits.push('volcanic');
  if (environment.boundaryType === 'divergent' && environment.boundaryProximity > 0.45) traits.push('rift');
  if (environment.sedimentaryBasinFactor > 0.62) traits.push('sedimentary');
  if (environment.reliefMeters > 1_250) traits.push('high-relief');
  if (surfaceType === 'land' && environment.meanElevationMeters > 1_500) traits.push('elevated');
  if (surfaceType === 'ocean' && coastDistance <= 2) traits.push('shelf');
  if (surfaceType === 'ocean' && coastDistance >= 2 && coastDistance <= 5) traits.push('slope');
  if (surfaceType === 'ocean' && environment.boundaryType === 'divergent' && environment.boundaryProximity > 0.52) traits.push('ridge');
  if (surfaceType === 'ocean' && environment.boundaryType === 'convergent' && environment.boundaryProximity > 0.58) traits.push('trench');
  if (surfaceType === 'ocean' && environment.meanElevationMeters < -3_400) traits.push('abyssal');
  return uniqueTraits(traits);
}

function classifyLand(environment: PlanetEnvironmentSample, coastDistance: number): { type: GeographicRegionType; confidence: number } {
  ...
  if (rift > 0.62) return { type: 'rift-zone', confidence: rift };
  if (volcanicArc > 0.68) return { type: 'volcanic-arc', confidence: volcanicArc };
  if (mountain > 0.62) return { type: 'mountain-range', confidence: mountain };
  if (coastDistance <= 2 && coastalHighlands > 0.66) return { type: 'coastal-highlands', confidence: coastalHighlands };
  if (coastDistance <= 2 && coastalPlain > 0.64) return { type: 'coastal-plain', confidence: coastalPlain };
  if (basin > 0.68) return { type: 'sedimentary-basin', confidence: basin };
  if (plateau > 0.7) return { type: 'plateau', confidence: plateau };
  if (highlands > 0.62) return { type: 'highlands', confidence: highlands };
  if (lowlands > 0.72) return { type: 'lowlands', confidence: lowlands };
  return { type: 'interior-plain', confidence: Math.max(0.42, lowRelief * 0.55 + (1 - highElevation) * 0.2) };
}

function classifyOcean(environment: PlanetEnvironmentSample, coastDistance: number): { type: GeographicRegionType; confidence: number } {
  ...
  if (trench > 0.64) return { type: 'oceanic-trench', confidence: trench };
  if (ridge > 0.6) return { type: 'mid-ocean-ridge', confidence: ridge };
  if (coastDistance <= 2 && shelf > 0.67) return { type: 'continental-shelf', confidence: shelf };
  if (coastDistance <= 5 && slope > 0.62) return { type: 'continental-slope', confidence: slope };
  if (plateau > 0.7) return { type: 'ocean-plateau', confidence: plateau };
  if (abyssal > 0.68) return { type: 'abyssal-plain', confidence: abyssal };
  return { type: 'ocean-basin', confidence: Math.max(0.44, deep * 0.28 + lowRelief * 0.22) };
}

function classifyPatch(patch: GeographyPatch, coastDistance: number, context: PlanetEnvironmentContext): PatchClassification {
...
function smoothClassifications(patches: readonly GeographyPatch[], graph: PatchGraph, values: PatchClassification[]): PatchClassification[] {
  ...
  for (let pass = 0; pass < 2; pass += 1) {
    ...
    for (let index = 0; index < patches.length; index += 1) {
      ...
      if (SIGNIFICANT_TYPES.has(value.geographicType) && value.confidence >= 0.78) continue;
      const counts = new Map<GeographicRegionType, { count: number; confidence: number; traits: GeographicTrait[] }>();
      ...
      if (best[1].count < 2 || best[1].count / Math.max(1, neighborCount) < 0.56) continue;
...
function componentKey(classification: PatchClassification): string {
...
function classificationComponents(patches: readonly GeographyPatch[], graph: PatchGraph, classifications: readonly PatchClassification[]): ClassificationComponent[] {
  ...
  for (let start = 0; start < patches.length; start += 1) {
    ...
    while (queue.length) {
      ...
      for (const neighbor of graph.neighbors[current]!) {
        ...
        if (neighborPatch.parentId !== patch.parentId || neighborPatch.surfaceType !== patch.surfaceType) continue;
        if (componentKey(classifications[neighbor]!) !== key) continue;
        visited[neighbor] = 1;
        queue.push(neighbor);
      }
    }
...
function farthestPatchIndexes(component: ClassificationComponent, patches: readonly GeographyPatch[], classifications: readonly PatchClassification[], count: number): number[] {
  ...
  while (selected.length < Math.min(count, component.indexes.length)) {
    ...
    for (const index of component.indexes) {
      ...
      for (const existing of selected) nearest = Math.min(nearest, wrappedDistanceSquared(patches[index]!.center, patches[existing]!.center));
      const weighted = nearest * (0.85 + classifications[index]!.confidence * 0.3);
      if (weighted > candidateDistance || (weighted === candidateDistance && index < candidate)) { candidate = index; candidateDistance = weighted; }
    }
...
function createProvinceSeeds(patches: readonly GeographyPatch[], graph: PatchGraph, components: readonly ClassificationComponent[], classifications: readonly PatchClassification[]): GeographicProvinceSeed[] {
  const seeds: GeographicProvinceSeed[] = [];
  const parentsWithSeed = new Set<string>();
  for (const component of components) {
    if (component.area < MIN_SEED_COMPONENT_AREA) continue;
  ...
  for (let start = 0; start < patches.length; start += 1) {
    ...
    while (queue.length) {
      const current = queue.pop()!;
      indexes.push(current);
      if (seededPatches.has(current)) hasSeed = true;
      for (const neighbor of graph.neighbors[current]!) {
        if (visited[neighbor]) continue;
        if (patches[neighbor]!.parentId !== patch.parentId || patches[neighbor]!.surfaceType !== patch.surfaceType) continue;
        visited[neighbor] = 1;
        queue.push(neighbor);
      }
    }
  ...
  pop(): HeapEntry | undefined {
    ...
    if (this.values.length) {
      ...
      while (true) {
        ...
        let child = left;
        if (right < this.values.length && this.less(this.values[right]!, this.values[left]!)) child = right;
...
function semanticTransitionMultiplier(seed: GeographicProvinceSeed, classification: PatchClassification): number {
  if (classification.geographicType === seed.geographicType) return 0.52;
  ...
  if (seedSignificant && targetSignificant) return 3.8;
  if (seedSignificant || targetSignificant) return 2.25;
  if (GENERIC_TYPES.has(seed.geographicType) && GENERIC_TYPES.has(classification.geographicType)) return 1.2;
  return 1.55;
}

function assignPatchesByGeographicAffinity(
  patches: readonly GeographyPatch[],
  graph: PatchGraph,
  classifications: readonly PatchClassification[],
  seeds: readonly GeographicProvinceSeed[],
): GeographicProvinceAssignment[] {
  ...
  while (heap.size) {
    ...
    if (current.distance !== distance[current.patchIndex] || owner[current.patchIndex] !== current.seedIndex) continue;
    ...
    for (const neighbor of graph.neighbors[current.patchIndex]!) {
      ...
      if (nextPatch.parentId !== seed.parentId || nextPatch.surfaceType !== seed.surfaceType) continue;
  ...
  const fallbackSeedByParent = new Map<string, number>();
  for (let index = 0; index < seeds.length; index += 1) if (!fallbackSeedByParent.has(seeds[index]!.parentId)) fallbackSeedByParent.set(seeds[index]!.parentId, index);
  const assignments = seeds.map(seed => ({ seed, patches: [] as GeographyPatch[] }));
  for (let index = 0; index < patches.length; index += 1) {
    let seedIndex = owner[index]!;
    if (seedIndex < 0) seedIndex = fallbackSeedByParent.get(patches[index]!.parentId) ?? -1;
    if (seedIndex < 0) throw new Error(`No semantic geographic province reaches patch ${patches[index]!.id}.`);
 ...
 */
export function generateGeographicProvinceAssignments(geography: GeneratedGeography, context: PlanetEnvironmentContext): GeographicProvinceAssignment[] {
