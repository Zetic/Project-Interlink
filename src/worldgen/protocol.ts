...
export interface WorldgenTopologyMetrics { sampleCount: number; edgeCount: number; faceCount: number; fiveNeighborCount: number; sixNeighborCount: number; totalAreaSteradians: number; minimumAreaSteradians: number; maximumAreaSteradians: number; meanAreaSteradians: number; areaCoefficientOfVariation: number; minimumEdgeArcRadians: number; maximumEdgeArcRadians: number; meanEdgeArcRadians: number; edgeCoefficientOfVariation: number; topologyHash: string; }
export interface WorldgenTopologyResult { engineVersion: number; level: number; metrics: WorldgenTopologyMetrics; durationMs: number; positions: Float64Array; faces: Uint32Array; neighborOffsets: Uint32Array; neighbors: Uint32Array; areaSteradians: Float64Array; birthLevels: Uint8Array; parentEdges: Uint32Array; }
