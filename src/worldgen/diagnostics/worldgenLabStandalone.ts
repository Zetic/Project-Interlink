...
import { WORLDGEN_BOUNDARY_CONVERGENT, WORLDGEN_BOUNDARY_DIVERGENT, WORLDGEN_BOUNDARY_TRANSFORM, type WorldgenTectonicsResult } from '../protocol.js';
...
function samplePosition(result: WorldgenTectonicsResult, sample: number): Vec3 { const offset = sample * 3; return [result.positions[offset]!, result.positions[offset + 1]!, result.positions[offset + 2]!]; }
...
function boundaryColor(kind: number): string { if (kind === WORLDGEN_BOUNDARY_CONVERGENT) return '#ff7272'; if (kind === WORLDGEN_BOUNDARY_DIVERGENT) return '#64d7ff'; if (kind === WORLDGEN_BOUNDARY_TRANSFORM) return '#ffd36a'; return '#d7e2ef'; }

function renderTectonics(canvas: HTMLCanvasElement, result: WorldgenTectonicsResult, projection: string, mode: string, yaw: number, pitch: number): void {
  const width = 1100;
  ...
  if (mode === 'plates' || mode === 'motion') {
    for (let plate = 0; plate < result.metrics.plateCount; plate += 1) {
      ...
      context.beginPath(); context.arc(x, y, Math.max(3.2, pointRadius + 1.6), 0, Math.PI * 2); context.stroke();
    }
  }
...
function renderMetrics(container: HTMLElement, result: WorldgenTectonicsResult): void {
...
function install(): void {
  ...
  let current: WorldgenTectonicsResult | null = null;
  let yaw = 0.0, pitch = 0.15, dragging = false, lastX = 0, lastY = 0;
  function redraw(): void { if (current) renderTectonics(canvas, current, projection.value, visualization.value, yaw, pitch); }
