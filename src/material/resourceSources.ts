...
const ORE_GRAIN_D50_RANGES_UM: Readonly<Record<string, readonly [number, number]>> = Object.freeze({
  'iron-ore': Object.freeze([45, 450]) as readonly [number, number],
  'copper-ore': Object.freeze([20, 250]) as readonly [number, number],
  'aluminum-ore': Object.freeze([60, 600]) as readonly [number, number],
});

const ORE_COMMINUTION_RANGES: Readonly<Record<string, Readonly<{ cwi: readonly [number, number]; bwi: readonly [number, number]; ai: readonly [number, number] }>>> = Object.freeze({
  'iron-ore': Object.freeze({ cwi: Object.freeze([6, 16]) as readonly [number, number], bwi: Object.freeze([9, 22]) as readonly [number, number], ai: Object.freeze([0.20, 0.65]) as readonly [number, number] }),
  'copper-ore': Object.freeze({ cwi: Object.freeze([7, 18]) as readonly [number, number], bwi: Object.freeze([10, 24]) as readonly [number, number], ai: Object.freeze([0.25, 0.75]) as readonly [number, number] }),
  'aluminum-ore': Object.freeze({ cwi: Object.freeze([4, 14]) as readonly [number, number], bwi: Object.freeze([7, 18]) as readonly [number, number], ai: Object.freeze([0.05, 0.35]) as readonly [number, number] }),
});
...
function validateTemplate(template: ResourceSourceTemplate, resourceId: string): void {
  ...
  for (const range of template.composition) requireMaterialSpecies(range.speciesId);
  ...
  if (minTotal > 1 + 1e-12 || maxTotal < 1 - 1e-12) throw new Error(`Resource '${resourceId}' composition bounds cannot sum to 100%.`);
}
...
function logUniform(rng: RangeRng, min: number, max: number): number {
  return Math.exp(rng.range(Math.log(min), Math.log(max)));
}

function normalizedOccurrenceModes(rng: RangeRng, complexity: number): MineralOccurrenceModes {
  const raw = {
...
function createMineralTexture(resourceId: string, composition: readonly MaterialComponentFraction[], rng: RangeRng): MineralTextureProfile | null {
  const d50Range = ORE_GRAIN_D50_RANGES_UM[resourceId];
  if (!d50Range) return null;
  const occurrenceD50Um = logUniform(rng, d50Range[0], d50Range[1]);
  const complexity = rng.range(0.15, 0.85);
  const speciesTextures: MineralTextureProfile['speciesTextures'] = {};
...
function createComminutionProperties(resourceId: string, rng: RangeRng): ComminutionProperties | null {
...
export function createResourceSource(resourceId: string, rng: RangeRng): ResourceSourceDefinition {
  ...
  if (!template) throw new Error(`No source template for resource '${resourceId}'.`);
  ...
  if (mineralTexture && comminutionProperties) mineralTexture.comminutionProperties = { ...comminutionProperties };
  return {
