import { listMaterialSpecies, requireMaterialSpecies, type MaterialSpeciesDefinition } from './species.js';
import type { MaterialPhysicalForm } from './types.js';

export interface ReactionParticipant {
  speciesId: string;
  stoichiometricMoles: number;
  physicalForm?: MaterialPhysicalForm;
}

export interface ArrheniusFirstOrderKinetics {
  model: 'arrhenius-first-order';
  activationEnergyJPerMol: number;
  preExponentialFactorPerSecond: number;
  referenceParticleSizeM?: number;
  particleSizeExponent?: number;
  minimumParticleSizeFactor?: number;
  maximumParticleSizeFactor?: number;
}
...
export interface ReactionDefinition {
  ...
  thermochemistry?: { reactionEnthalpyJPerMolExtent: number };
  kinetics?: ArrheniusFirstOrderKinetics;
}

function elementTotals(participants: readonly ReactionParticipant[]): Map<string, number> {
  ...
  for (const participant of participants) {
    ...
    if (!chemistry) throw new Error(`Reaction species '${participant.speciesId}' is missing elemental chemistry data.`);
...
export function validateReactionDefinition(reaction: ReactionDefinition): ReactionDefinition {
  if (!reaction.id || !reaction.name) throw new Error('Reaction definitions require id and name.');
  if (!reaction.reactants.length || !reaction.products.length) throw new Error(`Reaction '${reaction.id}' requires reactants and products.`);
...
export const GOETHITE_DEHYDROXYLATION_REACTION: ReactionDefinition = Object.freeze(validateReactionDefinition({
...
export const REACTION_DEFINITIONS: readonly ReactionDefinition[] = Object.freeze([
  GOETHITE_DEHYDROXYLATION_REACTION,
]);

export function elementsAvailableFromSpecies(speciesIds: readonly string[]): ReadonlySet<string> {
  const elements = new Set<string>();
  for (const speciesId of speciesIds) {
    const chemistry = requireMaterialSpecies(speciesId).chemistry;
    if (!chemistry) continue;
    for (const element of Object.keys(chemistry.elementalComposition)) elements.add(element);
  }
  return elements;
}
 ...
 */
export function candidateSpeciesForFeed(speciesIds: readonly string[]): readonly MaterialSpeciesDefinition[] {
