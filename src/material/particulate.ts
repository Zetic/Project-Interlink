...
export interface ParticleSizeBin {
  id: string;
  name: string;
  minMm: number;
  maxMm: number;
...
export interface LiberationClass {
  id: LiberationClassId;
  name: string;
  recoveryFactor: number;
}

export const PARTICLE_SIZE_BINS: readonly ParticleSizeBin[] = Object.freeze([
...
export const LIBERATION_CLASSES: readonly LiberationClass[] = Object.freeze([
...
const LEGACY_PARTICLE_SIZE_BIN_BY_ID = new Map<string, ParticleSizeBin & { index: number }>([
...
interface FragmentationPopulationTemplate {
  particleSizeBinId: string;
  liberationShares: Partial<Record<LiberationClassId, number>>;
  massShare: number;
}

const FRAGMENTATION_TEMPLATES: Readonly<Record<FragmentationProfileId, readonly FragmentationPopulationTemplate[]>> = Object.freeze({
...
export function particleSizeBinById(id: string): (ParticleSizeBin & { index: number }) | null {
  return PARTICLE_SIZE_BIN_BY_ID.get(id) ?? LEGACY_PARTICLE_SIZE_BIN_BY_ID.get(id) ?? null;
}

export function liberationClassById(id: LiberationClassId): (LiberationClass & { index: number }) | null {
  return LIBERATION_CLASS_BY_ID.get(id) ?? null;
}

export function validateMineralTextureProfile(profile: MineralTextureProfile): MineralTextureProfile {
  if (!profile.id || profile.id.includes('|')) throw new Error('Mineral texture profile requires a non-empty id without |.');
  const entries = Object.entries(profile.speciesTextures);
  if (!entries.length) throw new Error(`Mineral texture '${profile.id}' must contain species textures.`);
  for (const [speciesId, texture] of entries) {
    ...
    if (Math.abs(total - 1) > 0.005) throw new Error(`Mineral texture '${profile.id}' species '${speciesId}' occurrence modes must sum to 1.`);
  }
  return profile;
}

export function materializeSolidParticulateUnit(source: ResourceSourceDefinition): SolidParticulatePopulation[] {
  if (source.physicalForm !== 'solid-particulate') throw new Error(`Unsupported source physical form '${source.physicalForm}'.`);
  const template = FRAGMENTATION_TEMPLATES[source.fragmentationProfileId];
  if (!template) throw new Error(`Unknown fragmentation profile '${source.fragmentationProfileId}'.`);
  const textureProfileId = source.mineralTexture?.id ?? null;
  const populations: SolidParticulatePopulation[] = [];
  for (const component of source.composition) {
    for (const size of template) {
      if (!particleSizeBinById(size.particleSizeBinId)) throw new Error(`Unknown particle-size bin '${size.particleSizeBinId}'.`);
      for (const [liberationClassId, liberationShare] of Object.entries(size.liberationShares) as [LiberationClassId, number][]) {
        if (!liberationClassById(liberationClassId)) throw new Error(`Unknown liberation class '${liberationClassId}'.`);
        const massFraction = component.massFraction * size.massShare * liberationShare;
        if (massFraction <= 1e-12) continue;
        populations.push({
  ...
  if (Math.abs(total - 1) > 1e-8) throw new Error(`Materialized particulate source must total 1 kg; got ${total}.`);
  return populations;
}
