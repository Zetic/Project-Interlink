export type MaterialSpeciesKind =
  | 'mineral'
  | 'molecular-gas'
  | 'molecular-solid'
  | 'element'
...
export interface MaterialSpeciesThermalProperties {
  specificHeatCapacityJPerKgK: number;
}

export interface MaterialSpeciesChemistry {
  molarMassKgPerMol: number;
  elementalComposition: Readonly<Record<string, number>>;
}

export interface MaterialSpeciesDefinition {
  id: string;
  name: string;
  formula: string | null;
  kind: MaterialSpeciesKind;
  physicalProperties: {
    densityKgPerM3?: number;
    magneticResponse: { normalizedSeparationCoefficient: number };
    thermal?: MaterialSpeciesThermalProperties;
  };
  chemistry?: MaterialSpeciesChemistry;
}

function defineSpecies(definition: {
  id: string;
  name: string;
  formula?: string | null;
  kind?: MaterialSpeciesKind;
  ...
  thermal?: MaterialSpeciesThermalProperties;
  chemistry?: MaterialSpeciesChemistry;
}): MaterialSpeciesDefinition {
  const magneticResponse = definition.magneticResponse ?? 0;
  if (!definition.id) throw new Error('Material species id must be non-empty.');
  if (!Number.isFinite(magneticResponse) || magneticResponse < 0 || magneticResponse > 1) {
    throw new Error(`Material species '${definition.id}' magnetic response must be within [0, 1].`);
  }
  if (definition.densityKgPerM3 != null && (!Number.isFinite(definition.densityKgPerM3) || definition.densityKgPerM3 <= 0)) {
    throw new Error(`Material species '${definition.id}' density must be positive.`);
  }
  if (definition.thermal && (!Number.isFinite(definition.thermal.specificHeatCapacityJPerKgK) || definition.thermal.specificHeatCapacityJPerKgK <= 0)) {
    throw new Error(`Material species '${definition.id}' specific heat capacity must be positive.`);
  }
  if (definition.chemistry) {
    if (!Number.isFinite(definition.chemistry.molarMassKgPerMol) || definition.chemistry.molarMassKgPerMol <= 0) {
      throw new Error(`Material species '${definition.id}' molar mass must be positive.`);
    }
    for (const [element, atoms] of Object.entries(definition.chemistry.elementalComposition)) {
      if (!element || !Number.isFinite(atoms) || atoms <= 0) throw new Error(`Material species '${definition.id}' has invalid elemental composition.`);
    }
  }
  return Object.freeze({
    ...
    kind: definition.kind ?? 'mineral',
    physicalProperties: Object.freeze({
      ...(definition.densityKgPerM3 == null ? {} : { densityKgPerM3: definition.densityKgPerM3 }),
      magneticResponse: Object.freeze({ normalizedSeparationCoefficient: magneticResponse }),
      ...(definition.thermal ? { thermal: Object.freeze({ ...definition.thermal }) } : {}),
    }),
    ...(definition.chemistry ? {
      chemistry: Object.freeze({
        molarMassKgPerMol: definition.chemistry.molarMassKgPerMol,
        elementalComposition: Object.freeze({ ...definition.chemistry.elementalComposition }),
      }),
    } : {}),
  });
}

const SPECIES: readonly MaterialSpeciesDefinition[] = Object.freeze([
...
export const MATERIAL_SPECIES: Readonly<Record<string, MaterialSpeciesDefinition>> = Object.freeze(
  Object.fromEntries(SPECIES.map(species => [species.id, species])),
);

export function listMaterialSpecies(): readonly MaterialSpeciesDefinition[] {
  return SPECIES;
}

export function materialSpeciesById(speciesId: string): MaterialSpeciesDefinition | null {
  return MATERIAL_SPECIES[speciesId] ?? null;
}

export function requireMaterialSpecies(speciesId: string): MaterialSpeciesDefinition {
  const species = materialSpeciesById(speciesId);
