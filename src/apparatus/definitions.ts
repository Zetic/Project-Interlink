...
import type { NodePort, NodePortCapability } from '../world/types.js';

export interface ApparatusParameterChoice {
  value: number;
  label: string;
}
...
export interface ApparatusParameterDefinition {
  ...
  step: number;
  defaultValue: number;
  choices?: readonly ApparatusParameterChoice[];
}
...
export interface ApparatusDefinition {
  ...
  runtimeDefaults?: Readonly<Record<string, number>>;
}

const resourceInput = (id: string, label: string): NodePort => ({
  id, direction: 'input', kind: 'resource-access', medium: 'resource', label,
});
const solidInput = (id: string, label: string, accepts: readonly NodePortCapability[] = ['stored-solid-particulate']): NodePort => ({
  id, direction: 'input', kind: 'material', medium: 'solid', label, accepts,
});
const solidOutput = (id: string, label: string, provides: readonly NodePortCapability[] = ['solid-particulate']): NodePort => ({
  id, direction: 'output', kind: 'material', medium: 'solid', label, provides,
});
const gasInput = (id: string, label: string): NodePort => ({
  id, direction: 'input', kind: 'material', medium: 'gas', label, accepts: ['gas'],
});
const gasOutput = (id: string, label: string): NodePort => ({
  id, direction: 'output', kind: 'material', medium: 'gas', label, provides: ['gas'],
});
...
const define = (definition: ApparatusDefinition): ApparatusDefinition => Object.freeze({
  ...
  ports: Object.freeze(definition.ports.map(port => Object.freeze({
    ...port,
    ...(port.accepts ? { accepts: Object.freeze([...port.accepts]) } : {}),
    ...(port.provides ? { provides: Object.freeze([...port.provides]) } : {}),
  }))),
...
export const APPARATUS_DEFINITIONS: readonly ApparatusDefinition[] = Object.freeze([
  define({
    ...
    parameters: [{ id: 'rateKgPerSecond', label: 'Extraction rate', unit: 'kg/s', min: 0.001, step: 0.1, defaultValue: 5 }],
  }),
  define({
    ...
    parameters: [{ id: 'jawProductSizeMm', label: 'Nominal product size', unit: 'mm', min: 120, max: 250, step: 130, defaultValue: 120,
  ...
  define({
    ...
    physicalWidthMeters: 14, physicalHeightMeters: 9,
    ports: [solidInput('feed', 'feed'), solidOutput('product', 'product')],
    parameters: [{ id: 'coneProductSizeMm', label: 'Nominal product size', unit: 'mm', min: 5, max: 60, step: 5, defaultValue: 25,
  ...
  define({
    ...
    ports: [solidInput('feed', 'feed'), solidOutput('product', 'product')],
    parameters: [{ id: 'millProductSizeMm', label: 'Nominal product size', unit: 'mm', min: 0.032, max: 0.5, step: 0.001, defaultValue: 0.25,
  ...
  define({
    ...
    parameters: [{ id: 'apertureSizeMm', label: 'Aperture size', unit: 'mm', min: 1, max: 120, step: 1, defaultValue: 25,
  ...
  define({
    ...
    parameters: [{ id: 'splitFractionToA', label: 'Split to output A', unit: '', min: 0, max: 1, step: 0.01, defaultValue: 0.5 }],
  ...
  define({
    ...
    physicalWidthMeters: 12, physicalHeightMeters: 8,
    ports: [solidInput('feed', 'feed'), solidOutput('product', 'product', ['solid-particulate', 'metered-solid-particulate'])],
    parameters: [{ id: 'flowRateKgPerSecond', label: 'Feed rate', unit: 'kg/s', min: 0, max: 10, step: 0.1, defaultValue: 4 }],
    runtimeDefaults: { throughputKgPerSecond: 10 },
  }),
  define({
    ...
    parameters: [{ id: 'fieldStrength', label: 'Field strength', unit: '', min: 0, max: 1, step: 0.01, defaultValue: 0.6 }],
  ...
  define({
    ...
    ports: [
      solidInput('feed', 'feed', ['metered-solid-particulate']),
      solidOutput('solid-product', 'solid product', ['solid-particulate', 'metered-solid-particulate']),
      gasOutput('gas-exhaust', 'gas exhaust'),
    ],
    parameters: [{ id: 'temperatureSetpointK', label: 'Temperature setpoint', unit: 'K', min: 300, max: 1400, step: 10, defaultValue: 900 }],
    runtimeDefaults: { ratedHeaterPowerKw: 60, maximumOperatingTemperatureK: 1200, maximumSolidThroughputKgPerSecond: 4, effectiveChamberHoldUpKg: 20, heatLossCoefficientWPerK: 25, internalZoneCount: 4 },
  }),
  define({
  ...
  define({
    ...
    physicalWidthMeters: 12, physicalHeightMeters: 8,
    ports: [solidInput('input', 'in', ['solid-particulate']), solidOutput('output', 'out', ['stored-solid-particulate'])],
    parameters: [{ id: 'capacityKg', label: 'Capacity', unit: 'kg', min: 0.001, step: 10, defaultValue: 1000 }],
  }),
]);
