...
export function createThermalState({ sensibleEnthalpyJ = 0 } = {}) {
  assertFinite(sensibleEnthalpyJ, 'sensibleEnthalpyJ');
  return { sensibleEnthalpyJ };
}

export function cloneThermalState(thermalState = {}) {
  return createThermalState(thermalState);
}

export function validateThermalState(thermalState) {
...
export function thermalPropertyCoverageError(speciesIds) {
  return `Thermal property coverage missing for:\n${speciesIds.map(id => `- ${id}`).join('\n')}`;
}

export function heatCapacityJPerKForSpeciesMasses(speciesMassKg) {
  const missing = [];
  let capacity = 0;
...
export function temperatureKFromSensibleEnthalpy(sensibleEnthalpyJ, heatCapacityJPerK) {
  assertFinite(sensibleEnthalpyJ, 'sensibleEnthalpyJ');
...
export function sensibleEnthalpyJAtTemperature(temperatureK, heatCapacityJPerK) {
