...
import type { MapCameraState, MapSelection, Planet } from '../world/types.js';
...
function updateZoomVisibility(svg: SVGSVGElement, zoom: number): void {
  const resources = svg.querySelector<SVGGElement>('.ws-map-resource-layer');
  const resourceLabels = svg.querySelector<SVGGElement>('.ws-map-resource-label-layer');
  if (resources) resources.style.display = zoom < RESOURCE_MARKER_ZOOM ? 'none' : '';
  if (resourceLabels) resourceLabels.style.display = zoom < RESOURCE_LABEL_ZOOM ? 'none' : '';
  for (const marker of svg.querySelectorAll<SVGCircleElement>('.ws-map-resource')) {
  ...
  for (const label of svg.querySelectorAll<SVGTextElement>('.ws-map-resource-label')) {
