...
import type {
  RuntimeEntityDetail,
  RuntimeExhaustVentDetail,
  RuntimeFurnaceDetail,
  RuntimeHopperDetail,
...
let runtime: WasmPackedWorldRuntime | null = null;
let setup: FlatWorkerSetup | null = null;

function runtimeEvent(type: RuntimeEvent['type'], payload: RuntimeEvent['payload'], requestId?: number): RuntimeEvent {
  return {
...
function requireRuntime(): { runtime: WasmPackedWorldRuntime; setup: FlatWorkerSetup } {
  if (!runtime || !setup) throw new Error('Rust/WASM full flat runtime has not been initialized.');
  return { runtime, setup };
}

function normalizeOperatingState(value: string): RuntimeOperatingState {
  if (value === 'off' || value === 'idle' || value === 'running' || value === 'blocked') return value;
  return 'blocked';
}
...
function populateMaterialTables(target: WasmPackedWorldRuntime, nextSetup: FlatWorkerSetup): void {
  ...
  for (const sizeFactor of reaction.sizeFactors) target.set_reaction_size_factor(sizeFactor.sizeBinId, sizeFactor.factor);
...
function populateRuntime(target: WasmPackedWorldRuntime, nextSetup: FlatWorkerSetup): void {
  ...
  for (const source of nextSetup.occurrences) {
    target.add_occurrence_state(
      source.occurrenceId,
      source.speciesIds,
      source.sizeBinIds,
      source.liberationClassIds,
      source.textureProfileIds,
      source.quantitiesPerKg,
      source.reserveMassKg != null,
      source.reserveMassKg ?? 0,
    );
  }
  ...
  for (const machine of nextSetup.roastingFurnaces) {
    target.add_roasting_furnace(
      ...
      machine.nodeId,
      machine.ordinal,
      machine.temperatureSetpointK,
      machine.ratedHeaterPowerKw,
      ...
      machine.enabled,
      machine.productTargetKind,
      machine.productTargetId,
      machine.gasVentId,
    );
  }
  target.seal();
...
function runtimeMachineRows(currentSetup: FlatWorkerSetup): Array<{ nodeId: number; canonicalNodeId: string }> {
  return [
...
function snapshotWorld(target: WasmPackedWorldRuntime, currentSetup: FlatWorkerSetup): RuntimeSnapshot {
  ...
  const streams: RuntimeSnapshot['streams'] = {};
...
function profileSnapshot(target: WasmPackedWorldRuntime): RuntimeProfileSnapshot {
...
function furnaceDetail(target: WasmPackedWorldRuntime, currentSetup: FlatWorkerSetup, id: string): RuntimeFurnaceDetail {
  const furnace = currentSetup.roastingFurnaces.find(candidate => candidate.canonicalNodeId === id);
  if (!furnace) throw new Error(`Unknown runtime Furnace '${id}'.`);
  const zoneCount = Number(target.furnace_zone_count(furnace.nodeId)) || 0;
  const zoneMassKg: number[] = [];
  const zoneTemperatureK: Array<number | null> = [];
...
function ventDetail(target: WasmPackedWorldRuntime, currentSetup: FlatWorkerSetup, id: string): RuntimeExhaustVentDetail {
  ...
  const species = Array.from(target.exhaust_vent_species_ids(vent.nodeId)) as number[];
  const quantities = Array.from(target.exhaust_vent_quantities(vent.nodeId)) as number[];
  const compositionKg: Record<string, number> = {};
  quantities.forEach((quantity, index) => {
    if (!Number.isFinite(quantity) || quantity <= 0) return;
...
function detailFor(target: WasmPackedWorldRuntime, currentSetup: FlatWorkerSetup, entityType: unknown, id: unknown): RuntimeEntityDetail {
  if (typeof id !== 'string') throw new Error('Runtime detail query requires a canonical entity id.');
  if (entityType === 'hopper') return hopperDetail(target, currentSetup, id);
  if (entityType === 'furnace') return furnaceDetail(target, currentSetup, id);
  if (entityType === 'exhaustVent') return ventDetail(target, currentSetup, id);
  throw new Error(`Unsupported runtime detail entity '${String(entityType)}'.`);
}

function activeMachineIds(nextSetup: FlatWorkerSetup): Uint32Array {
  return Uint32Array.from(runtimeMachineRows(nextSetup).map(machine => machine.nodeId));
}

function reconfigureRuntime(target: WasmPackedWorldRuntime, nextSetup: FlatWorkerSetup): WasmPackedWorldRuntime {
  ...
  try {
    ...
    for (const hopper of nextSetup.hoppers) {
      candidate.replace_hopper_state_live(
        hopper.nodeId,
        hopper.capacityKg,
        candidate.hopper_species_ids(hopper.nodeId),
        candidate.hopper_size_bin_ids(hopper.nodeId),
        candidate.hopper_liberation_class_ids(hopper.nodeId),
        candidate.hopper_texture_profile_ids(hopper.nodeId),
        candidate.hopper_quantities(hopper.nodeId),
        candidate.hopper_sensible_enthalpy_j(hopper.nodeId),
      );
    }
    for (const vent of nextSetup.exhaustVents) {
      candidate.replace_exhaust_vent_state_live(
        vent.nodeId,
        candidate.exhaust_vent_species_ids(vent.nodeId),
        candidate.exhaust_vent_quantities(vent.nodeId),
        candidate.exhaust_vent_sensible_enthalpy_j(vent.nodeId),
      );
    }
    for (const machine of nextSetup.extractors) {
...
async function handle(command: RuntimeCommand): Promise<RuntimeEvent> {
  ...
  switch (command.type) {
    case 'init': {
      if (runtime) throw new Error('Rust/WASM full flat runtime is already initialized.');
      const nextSetup = command.payload.setup as FlatWorkerSetup;
    ...
    case 'advance-fixed': {
      ...
      if (!Number.isInteger(requested) || requested < 0 || requested > 10_000) throw new Error('advance-fixed steps must be an integer between 0 and 10000.');
    ...
    case 'set-profiling': {
      ...
      if (command.payload.reset === true) current.runtime.reset_profiling_stats();
...
interface RuntimeWorkerScope {
  ...
  postMessage(message: RuntimeEvent): void;
}

const workerScope = self as unknown as RuntimeWorkerScope;
