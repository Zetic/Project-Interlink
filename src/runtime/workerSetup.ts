...
export interface FlatWorkerHopper {
  nodeId: number;
  canonicalNodeId: string;
  capacityKg: number;
}

export interface FlatWorkerExtractor {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
...
export interface FlatWorkerStream {
  streamId: string;
...
function fragmentationRuntimeId(profileId: string, table: Map<string, number>, values: string[]): number {
  const existing = table.get(profileId);
  if (existing != null) return existing;
  const next = values.length;
  if (next > 0xff) throw new Error('Flat runtime fragmentation ID capacity exceeded.');
  table.set(profileId, next);
  values.push(profileId);
  return next;
}

function speciesRuntimeId(speciesId: string, table: Map<string, number>, values: string[]): number {
  const existing = table.get(speciesId);
  if (existing != null) return existing;
  const next = values.length;
  if (next > 0xffff) throw new Error('Flat runtime species ID capacity exceeded.');
  table.set(speciesId, next);
  values.push(speciesId);
  return next;
}

function extractorOutputStream(
  streams: readonly RuntimeMaterialStreamBinding[],
  extractorRuntimeId: number,
): RuntimeMaterialStreamBinding | null {
...
export function compileFlatWorkerSetup(plan: FlatRuntimePlan): FlatWorkerSetup {
  const speciesTable = new Map<string, number>();
  const speciesIds: string[] = [];
  const sizeBinTable = new Map<string, number>();
  const sizeBinIds: string[] = [];
  ...
  const hoppers: FlatWorkerHopper[] = plan.machines
  ...
  const extractors: FlatWorkerExtractor[] = plan.machines
