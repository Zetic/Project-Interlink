...
import type { RuntimeHopperDetail, RuntimeOperatingState, RuntimeProfileSnapshot, RuntimeSnapshot } from './presentation.js';
import {
...
function requireRuntime(): { runtime: WasmPackedWorldRuntime; setup: FlatWorkerSetup } {
  if (!runtime || !setup) throw new Error('Rust/WASM flat runtime has not been initialized.');
...
function normalizeOptionalMass(value: number): number | null {
...
function populateRuntime(target: WasmPackedWorldRuntime, nextSetup: FlatWorkerSetup): void {
  ...
  for (const extractor of nextSetup.extractors) {
    target.add_extractor(
      ...
      extractor.nodeId,
      extractor.ordinal,
      extractor.rateKgPerSecond,
      extractor.enabled,
      extractor.occurrenceId,
      extractor.outputHopperId,
    );
  }
  target.seal();
}
...
async function handle(command: RuntimeCommand): Promise<RuntimeEvent> {
  ...
  switch (command.type) {
    case 'init': {
      if (runtime) throw new Error('Rust/WASM flat runtime is already initialized.');
