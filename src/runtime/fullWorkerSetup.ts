...
import {
  NO_RUNTIME_ID,
  compileFlatWorkerSetup as compileExtractionWorkerSetup,
  type FlatWorkerSetup as ExtractionWorkerSetup,
  type FlatWorkerStream,
} from './workerSetup.js';
...
export interface FlatWorkerComminution {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  equipmentKind: number;
  targetSizeId: number;
  targetParticleSizeMm: number;
  throughputKgPerSecond: number;
  ratedPowerKw: number;
  enabled: boolean;
  inputHopperId: number;
  outputHopperId: number;
}

export interface FlatWorkerScreen {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  apertureSizeMm: number;
  throughputKgPerSecond: number;
  enabled: boolean;
  inputHopperId: number;
  undersizeHopperId: number;
  oversizeHopperId: number;
}

export interface FlatWorkerSplitter {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  splitFractionToA: number;
  throughputKgPerSecond: number;
  enabled: boolean;
  inputHopperId: number;
  outputAHopperId: number;
  outputBHopperId: number;
}

export interface FlatWorkerMerger {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  throughputKgPerSecond: number;
  enabled: boolean;
  inputAHopperId: number;
  inputBHopperId: number;
  outputHopperId: number;
}

export interface FlatWorkerFeeder {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  flowRateKgPerSecond: number;
  throughputKgPerSecond: number;
  enabled: boolean;
  inputHopperId: number;
  outputTargetKind: number;
  outputTargetId: number;
}

export interface FlatWorkerMagneticSeparator {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  fieldStrength: number;
  maxFeedParticleSizeMm: number;
  throughputKgPerSecond: number;
  enabled: boolean;
  inputHopperId: number;
  concentrateHopperId: number;
  tailingsHopperId: number;
}

export interface FlatWorkerRoastingFurnace {
  nodeId: number;
  canonicalNodeId: string;
  ordinal: number;
  temperatureSetpointK: number;
  ratedHeaterPowerKw: number;
  maximumOperatingTemperatureK: number;
  maximumSolidThroughputKgPerSecond: number;
  effectiveChamberHoldUpKg: number;
  heatLossCoefficientWPerK: number;
  internalZoneCount: number;
  enabled: boolean;
  productTargetKind: number;
  productTargetId: number;
  gasVentId: number;
}

export interface FlatWorkerExhaustVent {
  nodeId: number;
  canonicalNodeId: string;
}

export interface FullWorkerStream extends FlatWorkerStream {
  sourcePortId: string;
  targetPortId: string;
  physicalForm: 'solid-particulate' | 'gas';
  measurementNodeId: number;
  measurementDirection: 'input' | 'output';
  measurementPortIndex: number;
}

export interface FlatWorkerSetup extends ExtractionWorkerSetup {
  comminution: FlatWorkerComminution[];
  screens: FlatWorkerScreen[];
  splitters: FlatWorkerSplitter[];
  mergers: FlatWorkerMerger[];
  feeders: FlatWorkerFeeder[];
  magneticSeparators: FlatWorkerMagneticSeparator[];
  roastingFurnaces: FlatWorkerRoastingFurnace[];
  exhaustVents: FlatWorkerExhaustVent[];
  streams: FullWorkerStream[];
}
...
function inboundStream(streams: readonly RuntimeMaterialStreamBinding[], nodeId: number, portId: string): RuntimeMaterialStreamBinding | null {
  return streams.find(stream => stream.targetRuntimeId === nodeId && stream.targetPortId === portId) ?? null;
}

function outboundStream(streams: readonly RuntimeMaterialStreamBinding[], nodeId: number, portId: string): RuntimeMaterialStreamBinding | null {
  return streams.find(stream => stream.sourceRuntimeId === nodeId && stream.sourcePortId === portId) ?? null;
}

function hopperEndpoint(
  stream: RuntimeMaterialStreamBinding | null,
  side: 'source' | 'target',
  machineByRuntimeId: ReadonlyMap<number, RuntimeMachinePlan>,
): number {
  if (!stream) return NO_RUNTIME_ID;
...
function solidTarget(
  stream: RuntimeMaterialStreamBinding | null,
  machineByRuntimeId: ReadonlyMap<number, RuntimeMachinePlan>,
): { kind: number; id: number } {
  if (!stream) return { kind: SOLID_TARGET_NONE, id: NO_RUNTIME_ID };
  const target = machineByRuntimeId.get(stream.targetRuntimeId);
  if (target?.nodeType === 'hopper') return { kind: SOLID_TARGET_HOPPER, id: stream.targetRuntimeId };
  if (target?.nodeType === 'roastingFurnace') return { kind: SOLID_TARGET_FURNACE, id: stream.targetRuntimeId };
  return { kind: SOLID_TARGET_NONE, id: NO_RUNTIME_ID };
}

function exhaustVentTarget(
  stream: RuntimeMaterialStreamBinding | null,
  machineByRuntimeId: ReadonlyMap<number, RuntimeMachinePlan>,
): number {
  if (!stream) return NO_RUNTIME_ID;
...
function targetSizeId(setup: ExtractionWorkerSetup, targetMm: number): number {
  if (targetMm === 1) return setup.sizeBinIds.indexOf('lt-1mm');
  const exact = setup.materialTables.sizeBins.find(row => row.canonical && Math.abs(row.maxMm - targetMm) <= 1e-12);
  if (exact) return exact.runtimeId;
...
function machineOrdinal(plan: FlatRuntimePlan, runtimeId: number): number {
  return Math.max(0, plan.machines.findIndex(machine => machine.runtimeId === runtimeId));
}

function outputPortIndex(nodeType: string, portId: string): number | null {
  if (nodeType === 'screen') return portId === 'undersize' ? 0 : portId === 'oversize' ? 1 : null;
  if (nodeType === 'splitter') return portId === 'output-a' ? 0 : portId === 'output-b' ? 1 : null;
  if (nodeType === 'magSep') return portId === 'concentrate' ? 0 : portId === 'tailings' ? 1 : null;
  if (nodeType === 'roastingFurnace') return portId === 'solid-product' ? 0 : portId === 'gas-exhaust' ? 1 : null;
  if (nodeType === 'extractor' && portId === 'output') return 0;
  if ((nodeType === 'jawCrusher' || nodeType === 'coneCrusher' || nodeType === 'ballMill' || nodeType === 'merger' || nodeType === 'feeder') && portId === 'product') return 0;
  return null;
}

function inputPortIndex(nodeType: string, portId: string): number | null {
  if (nodeType === 'merger') return portId === 'input-a' ? 0 : portId === 'input-b' ? 1 : null;
  if (nodeType === 'exhaustVent' && portId === 'gas-in') return 0;
  if (nodeType === 'roastingFurnace' && portId === 'feed') return 0;
  if ((nodeType === 'jawCrusher' || nodeType === 'coneCrusher' || nodeType === 'ballMill' || nodeType === 'screen' || nodeType === 'splitter' || nodeType === 'feeder' || nodeType === 'magSep') && portId === 'feed') return 0;
  if (nodeType === 'hopper' && portId === 'input') return 0;
  return null;
}

function supportedStream(
  stream: RuntimeMaterialStreamBinding,
  machineByRuntimeId: ReadonlyMap<number, RuntimeMachinePlan>,
): FullWorkerStream {
  ...
  const runtimeSupported = sourceOutputSupported || targetInputSupported || (
    source?.nodeType === 'roastingFurnace' && target?.nodeType === 'exhaustVent' && sourceIndex === 1
  );
  return {
...
export function compileFlatWorkerSetup(plan: FlatRuntimePlan): FlatWorkerSetup {
  ...
  const comminution: FlatWorkerComminution[] = plan.machines
  ...
  const screens: FlatWorkerScreen[] = plan.machines.filter(machine => machine.nodeType === 'screen').map(machine => ({
    nodeId: machine.runtimeId,
    canonicalNodeId: machine.nodeId,
    ordinal: machineOrdinal(plan, machine.runtimeId),
  ...
  const splitters: FlatWorkerSplitter[] = plan.machines.filter(machine => machine.nodeType === 'splitter').map(machine => ({
    nodeId: machine.runtimeId,
    canonicalNodeId: machine.nodeId,
    ordinal: machineOrdinal(plan, machine.runtimeId),
  ...
  const mergers: FlatWorkerMerger[] = plan.machines.filter(machine => machine.nodeType === 'merger').map(machine => ({
    nodeId: machine.runtimeId,
    canonicalNodeId: machine.nodeId,
    ordinal: machineOrdinal(plan, machine.runtimeId),
    throughputKgPerSecond: numberParameter(machine, 'throughputKgPerSecond', 10),
  ...
  const feeders: FlatWorkerFeeder[] = plan.machines.filter(machine => machine.nodeType === 'feeder').map(machine => {
  ...
  const magneticSeparators: FlatWorkerMagneticSeparator[] = plan.machines.filter(machine => machine.nodeType === 'magSep').map(machine => ({
    nodeId: machine.runtimeId,
    canonicalNodeId: machine.nodeId,
    ordinal: machineOrdinal(plan, machine.runtimeId),
  ...
  const roastingFurnaces: FlatWorkerRoastingFurnace[] = plan.machines.filter(machine => machine.nodeType === 'roastingFurnace').map(machine => {
  ...
  const exhaustVents: FlatWorkerExhaustVent[] = plan.machines.filter(machine => machine.nodeType === 'exhaustVent').map(machine => ({
    nodeId: machine.runtimeId,
    canonicalNodeId: machine.nodeId,
  }));
...
function structureMachine(machine: { nodeId: number; canonicalNodeId: string; ordinal: number }, endpoints: Record<string, number>): object {
