...
const workerUrl = new URL('../src/simulation/rustWasmWorker.js', import.meta.url);
...
test('interlink-wasm source exposes only the world runtime bridge', () => {
  ...
  for (const stale of removedStandaloneClasses) {
    assert.doesNotMatch(lib, new RegExp(stale));
  }
});
...
test('generated browser API does not expose standalone apparatus/material WASM objects', () => {
  ...
  for (const stale of removedStandaloneClasses) {
    assert.doesNotMatch(declarations, new RegExp(`export class ${stale}\\b`));
  }
});

test('production Worker imports only the world runtime and protocol version', () => {
